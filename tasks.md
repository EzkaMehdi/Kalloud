...
- [x] **`STK-07` — Ajouter l’inventaire physique**
  - Priorité : `P1`
  - Dépend de : `STK-04`
  ...
  - Mise en œuvre : le comptage obtient sa **propre table** (`migrations/0018`, `stock_counts`) plutôt que de vivre entièrement dans le mouvement qu'il produit, pour deux raisons que l'acceptation rend concrètes. (1) Un mouvement enregistre un **delta** : « stock avant » et « compté » seraient à reconstruire en rejouant le registre jusque-là — dérivables, mais pas *consultables*. (2) Surtout, **un comptage juste ne produit aucun mouvement** : `quantity <> 0` (`migrations/0007`) interdit une ligne à quantité nulle, à raison — or « j'ai compté, et le stock était juste » est précisément ce qu'un inventaire sert à établir, et sans cette table ce comptage ne laisserait aucune trace. La colonne `difference` est **générée** par la base (`counted − theoretical`) : un écart en désaccord avec ses propres opérandes est la seule chose qu'un inventaire n'a pas le droit de produire. `lib/services/stock.ts::recordStockCount` lit le solde théorique **sous le verrou** avec lequel la correction est écrite, jamais avant : une vente encaissée pendant le comptage rendrait sinon le « stock avant » déjà faux au moment où on l'inscrit, et l'écart accuserait le compteur de la transaction de quelqu'un d'autre. Quand l'écart est non nul, un mouvement `CORRECTION` est écrit avec `reference_type = 'stock_count'` et l'identifiant du comptage — les colonnes `reference_type`/`reference_id` posées par `STK-01` et jamais encore utilisées trouvent ici leur premier emploi — et le comptage garde en retour l'identifiant du mouvement, de sorte que la paire se lit des deux côtés. Le type est `CORRECTION` quelle que soit la direction : c'est le seul que `DEC-06` autorise dans les deux sens, et le seul dont le sens métier est « le registre avait tort, voici le rattrapage ». Côté écran, `components/stock-count-modal.tsx` (nouveau) est délibérément **distinct** du dialogue d'ajustement : un ajustement dit « bouge le solde de tant », un comptage dit « le solde *est* tant » — le seul endroit où un nombre absolu est l'entrée honnête — et les réunir mettrait la question dangereuse (« quel est le nouveau total ? ») à côté de celle qu'on peut poser sans risque. L'écart est annoncé **avant** validation, et l'historique sous le formulaire porte les cinq champs de l'acceptation. Testé par `tests/integration/stock-count.test.ts` (7 cas, dont le comptage juste qui n'invente pas de mouvement, la double référence croisée, et le solde théorique lu au bon moment malgré une vente concurrente) et `tests/e2e/stock-count.spec.ts` (3 cas, dont la consultation des cinq figures). Deux corrections d'intendance : le `globalTeardown` e2e supprimait les mouvements avant les comptages qui les référencent, et les specs `STK-05`/`STK-06` désignaient « le bouton de la ligne » alors qu'il y en a désormais deux.

- [x] **`STK-08` — Fiabiliser la liste et les alertes**
  - Priorité : `P1`
  ...
  - Mise en œuvre : audit des cinq volets. **Données réelles** et **alertes sur le solde serveur** étaient acquis depuis `UX-01` (la liste lit `/api/products`, les badges et le compteur d'alertes sont une lecture de ce que le serveur renvoie, pas un état local). La promesse « **temps réel** » du prototype (`Inventaire en temps réel`, alors que `DEC-08` exclut tout push live au MVP) avait été retirée par `UX-01` — ce qui manquait n'était pas le retrait mais le **garde-fou** : un test vérifie désormais que les mots ne reviennent pas, l'acceptation étant formulée comme une interdiction. Deux volets étaient réellement ouverts. (1) **État vide** : la page confondait catalogue vide et recherche infructueuse, si bien qu'un établissement neuf lisait « Aucun produit ne correspond à «  » » — un résultat de recherche pour une recherche que personne n'a lancée, qui se lit comme un bug plutôt que comme une étape d'installation. Les deux faits sont désormais distincts, et le message du catalogue vide indique où créer un produit. (2) **Resynchronisation multi-appareil** : la liste était lue une fois au montage et restait figée pendant qu'un autre appareil modifiait les mêmes données — une réception comptée sur une tablette laissait l'écran de la caisse silencieusement faux. `DEC-08` excluant le push live, le compromis honnête est celui que `useCurrentUser` applique déjà à la session (`UX-03`, Mehdi) : relire quand l'onglet revient au premier plan, qui est le dernier instant avant que quelqu'un agisse sur ce qu'il voit. L'option `revalidateOnFocus` est ajoutée à `useAsyncData` et **désactivée par défaut** : un écran qui se rafraîchit doit être un écran dont les données peuvent changer sous lui, et le dire page par page en fait une décision plutôt qu'une habitude. Seul l'écran stock l'active pour l'instant ; la caisse en bénéficierait aussi, mais c'est hors du livrable de cette tâche. Testé par `tests/e2e/stock-list.spec.ts` (nouveau, 4 cas, sur un établissement jetable — l'un d'eux exige un catalogue vide, ce que le tenant seedé ne peut jamais être). Le cas de resynchronisation a été validé par mutation : option désactivée, il échoue exactement sur l'assertion finale.

- [x] **`STK-09` — Tester le ledger de stock**
  - Priorité : `P0`
  ...
  - Mise en œuvre : audit préalable des quatre angles. **Vente** (`STK-03`), **ajustement** (`STK-04`) et **concurrence au sein d'une même opération** — deux décréments qui se disputent le dernier article, deux ajustements simultanés — étaient déjà couverts et ne sont pas redits. Deux choses ne l'étaient pas. (1) L'**invariant énoncé comme invariant** : chaque opération l'affirmait pour le seul produit qu'elle touche, jamais sur *tous* les produits après une charge mêlant tous les chemins d'écriture. Une assertion par opération prouve que chacune tient ses comptes ; seule une assertion globale prouve qu'elles les tiennent **ensemble**, et c'est là qu'un verrou manquant ou une écriture hors de `recordStockMovement` se verrait. (2) La **concurrence entre opérations différentes** — une caisse qui encaisse pendant qu'un responsable compte — qui est la course qu'un vrai service produit, alors que les tests existants ne faisaient courir que deux opérations de même nature. `tests/integration/stock-ledger-invariant.test.ts` (nouveau, 5 cas) : une charge mixte (vente, remboursement, réception, perte, correction, deux comptages dont un conforme, plus un produit sans aucun mouvement — le cas limite « zéro des deux côtés ») ; une vente contre un ajustement ; une vente contre un comptage physique, où le solde final doit appartenir à l'un des deux ordonnancements possibles et jamais à un troisième ; et une rafale de cinq écritures concurrentes sur le même produit, dont le total prouve qu'aucune n'a été perdue ni appliquée deux fois. Le cinquième cas **garde le garde** : il corrompt délibérément un solde par un `UPDATE products` direct — exactement l'écriture dont `STK-04` a supprimé l'endpoint — et vérifie que le contrôle le détecte et nomme le produit fautif. Un contrôle d'invariant qui répond toujours « aucune anomalie » ne rassure que s'il sait répondre autre chose.

- [x] **`STK-10` — Tester les opérations et l’interface de stock**
  - Priorité : `P1`
  ...
  - Mise en œuvre : les **sept volets du livrable** sont déjà pilotés depuis l'interface par les specs écrites avec leur propre tâche (`stock-adjustment` pour réception/perte/correction/retour, `stock-count` pour l'inventaire, `stock-list` pour alertes, état vide et resynchronisation) ; les répéter aurait ajouté des fichiers, pas de la couverture. Ce qu'aucune n'affirmait est la **correspondance** que l'acceptation demande : elles vérifient que le solde a bougé, pas que ce qu'une personne a fait à l'écran a laissé une trace nommant l'acte, son auteur et son instant — la différence entre un stock qui a changé et un stock dont on peut rendre compte. `tests/e2e/stock-auditability.spec.ts` (nouveau) pilote les quatre opérations depuis le dialogue et vérifie, pour chacune, le mouvement de registre **et** l'événement d'audit correspondant, dans l'ordre d'exécution et avec les mots de l'opérateur ; puis les deux comptages, dont celui qui tombe juste — une seule ligne de registre pour deux comptages, mais **deux** traces d'audit, parce que « j'ai compté, rien n'était faux » est une opération et doit se rendre compte ; puis un refus serveur, qui ne laisse **ni** mouvement **ni** audit, un journal qui consignerait les tentatives rendant chaque entrée réelle plus difficile à croire. La trace est lue directement en base : `listAuditEvents` existe mais aucun endpoint ne l'expose encore (`DEC-07` accorde le droit de la consulter, l'écran est du travail ultérieur). **Bug trouvé en vérifiant `GATE-5` plutôt qu'en le supposant** : `scripts/seed.mjs` insérait `stock_quantity` en SQL brut **sans** le mouvement `OPENING_BALANCE` qui l'explique, si bien qu'une base fraîchement seedée violait l'invariant de `STK-09` dès le premier `pnpm db:seed` — neuf produits dont aucun mouvement ne justifiait le solde. Le seed était le troisième écrivain, manqué par la migration de rattrapage (`STK-02`) comme par le service (`createProductWithInitialStock`), exactement la même classe d'oubli que la catégorie des mouvements de caisse en `CASH-03`. Corrigé, et gardé par un test **au niveau e2e** : c'est le seul tier qui tourne sur la base seedée, la suite d'intégration construisant ses propres établissements et ne voyant donc jamais le seed.
...
- [x] Tout stock courant est reconstructible depuis son ledger. (`STK-01`/`STK-04`/`STK-07` — toute écriture passe par `recordStockMovement` ; `STK-09` énonce l'invariant sur tous les produits, et `STK-10` le garde sur la base seedée elle-même)
- [x] Caisse et Stock peuvent être utilisés sans `prompt()`, `alert()` ou données fictives. (`CASH-07`/`CASH-09` — journal réel borné au service ; `STK-05` — le `prompt()` remplacé par un vrai dialogue, vérifié à chaque test e2e ; `STK-08` — catalogue vide et recherche infructueuse distingués)

## 13. Phase 6 — Cockpit gérant

- [x] **`BI-01` — Implémenter le contrat des métriques**
  - Priorité : `P0`
  ...
  - Mise en œuvre : `lib/metrics/dictionary.ts` transcrit le dictionnaire de `DEC-09` en artefact du code — sept identifiants (`net_revenue`, `orders_count`, `average_basket`, `expected_cash`, `cash_variance`, `stock_out_of_stock`, `stock_low_stock`), chacun avec un `version` (figé à `1`, prévu pour être incrémenté le jour où une formule change de *sens*, pas de bug), un libellé, la formule en toutes lettres et ses tables source — pour qu'un export ou un ticket de support puisse un jour dire « ce chiffre vient d'une autre définition » plutôt que « ce chiffre est faux ». `lib/services/metrics.ts::getMetrics` calcule les sept et les enveloppe individuellement dans `{ id, version, label, source, value, period, timezone, computedAt }` — littéralement les quatre champs de l'acceptation, plus l'identité et la formule. Aucune formule n'est réimplémentée : le service appelle les mêmes fonctions de dépôt déjà éprouvées (`getBusinessDaySummary`, `getRevenueBetween`, `getExpectedCash`) plutôt que d'écrire une deuxième requête SQL qui pourrait un jour diverger de la première — le risque exact que `CASH-04` avait déjà fermé une fois pour les espèces attendues. Deux pièces manquaient réellement et sont nouvelles : `lib/repositories/products.ts::getStockAlertCounts` (compte rupture/sous-seuil scopé et testé, plutôt que dérivé côté client de la liste complète comme le fait `STK-08` pour ses badges) et `lib/repositories/business-days.ts::getLastClosedBusinessDay` (l'écart de la dernière clôture, symétrique de `getLastNextOpeningCash` déjà existant). Chaque métrique porte son propre type de période — `DEC-09` fixe les alertes de stock en « Instantané » et l'écart de caisse en « À la clôture » indépendamment de la période demandée par l'appelant, donc la période est une propriété de la définition de chaque KPI, pas de la requête ; un type `{ kind: "none" }` explicite remplace un zéro qui ressemblerait sinon à une vraie mesure quand rien n'a de sens à répondre (aucun service ouvert, aucune journée jamais clôturée). Aucune route n'appelle ce service pour l'instant — `BI-02` et suivants (historique, comparaison, bloc de rapprochement, bloc stock à risque) en seront les premiers consommateurs ; le livrable de cette tâche est le service et ses tests, pas un écran. Testé par `tests/integration/metrics.test.ts` (nouveau, 8 cas) : les sept métriques portent bien les champs de la dictionnaire (id/version/libellé/source, pas une copie qui pourrait diverger) sur un établissement neuf sans aucune donnée ; le fuseau horaire réellement configuré de l'établissement se retrouve sur une métrique périodique et sur une métrique instantanée ; CA net/nombre de commandes/panier moyen se réconcilient avec deux ventes réelles (espèces + carte) ; la période bascule en `range` hors `period=day` pendant que les espèces attendues restent honnêtement absentes (propriété d'une session, pas d'un mois) ; les espèces attendues suivent la formule de `CASH-04` ; l'écart de caisse lit la dernière clôture et reste `null` avant toute clôture ; le compte d'alertes de stock ignore un produit désactivé qui aurait autrement compté deux fois ; et l'isolation multi-établissement ne laisse fuiter ni vente ni alerte de stock d'un autre établissement. Suite complète revérifiée (`lint`/`typecheck`/`format`/442 tests unit+intégration [434 + 8 nouveaux]/75 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-02` — Créer les requêtes d’historique filtrées**
  - Priorité : `P0`
  ...
  - Mise en œuvre : audit préalable des cinq domaines cités par le livrable. « Commandes » était déjà entièrement couvert par `listOrderHistory` (`ORD-12` : filtrable, paginé, compté) — non dupliqué ici. Les quatre autres n'avaient, chacun, qu'une lecture bien plus étroite : `listCashMovements` (`CASH-07`) ne répond que pour le service **ouvert** (`[]` sans service actif, par choix délibéré de cette tâche) ; `listStockMovements` (`STK-01`) ne lit qu'**un seul produit** à la fois ; `listPaymentsForOrder` (`SALE-02`) ne lit qu'**une seule commande** ; et rien ne lisait `order_items` à l'échelle de l'établissement (question que `BI-08`, plus tard, posera au niveau produit/catégorie). Quatre nouvelles fonctions de dépôt, toutes au même format que `listOrderHistory` (filtres + `limit`/`offset` → `{ rows, total, limit, offset }`) : `listSoldItems` (« ventes » — une ligne par article vendu, commandes `PAID` **et** `REFUNDED` incluses puisqu'un remboursement annule le paiement, jamais la vente elle-même ; jointure obligatoire vers `orders` car `order_items` ne porte aucune colonne `location_id` propre, `SEC-06`), `listPaymentsHistory` (« paiements »), `listCashMovementsHistory` (« caisse » — délibérément distincte de `listCashMovements` : élargir cette dernière aurait soit rompu son contrat de journal du service en cours, soit réintroduit l'ambiguïté « mouvements d'hier sous le solde d'aujourd'hui » que `CASH-07` avait justement fermée), et `listStockMovementsHistory` (« stock », tous produits confondus). Quatre routes exposées (`GET /api/sales`, `/api/payments`, `/api/cash-movements/history`, `/api/stock-movements`), réservées à `dashboard:view` (`OWNER`/`MANAGER`) — contrairement à `/api/orders`/`/api/cash-movements` (journal en direct) qu'un caissier consulte pour son propre service, ces quatre lectures sont le détail de reporting de la Phase 6, jamais un geste opérationnel. « Performances mesurées » pris au pied de la lettre plutôt que traité comme une formule : `migrations/0019_history_query_indexes.sql` ajoute un index composé `(location_id, ...DESC)` pour chacune des quatre requêtes (`orders (location_id, status, paid_at DESC)`, `payments`/`cash_movements`/`stock_movements (location_id, created_at DESC)`), et `tests/integration/history-queries.test.ts` le prouve avec `EXPLAIN (FORMAT JSON)` sous `enable_seqscan = OFF` plutôt que sur un volume de données arbitraire — sur un jeu de test de quelques lignes, le planificateur choisit un balayage séquentiel de toute façon (moins coûteux qu'un index pour une si petite table), donc chronométrer l'exécution n'aurait rien prouvé ; la question mesurable est « cette requête peut-elle emprunter l'index que cette tâche ajoute », pas « le fait-elle sur ce jeu de données précis ». Testé par `tests/integration/history-queries.test.ts` (16 cas) : chaque domaine filtre correctement (produit, plage de dates, méthode, catégorie, type), pagine avec un total exact, et n'agrège jamais les données d'un autre établissement ; plus les quatre vérifications d'index. `tests/e2e/auth.spec.ts` étendu (pas un nouveau fichier) pour prouver que les quatre routes refusent un `CASHIER` avec `403`, au même endroit que la vérification déjà existante sur `/api/dashboard`. Suite complète revérifiée (`lint`/`typecheck`/`format`/466 tests unit+intégration [dont les 16 nouveaux de cette tâche]/75 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-03` — Clarifier service et périodes calendaires**
  - Priorité : `P0`
  ...
  - Mise en œuvre : `DEC-09` liste cinq valeurs de période distinctes (« Service en cours / jour / mois / année / plage »), mais `BI-01` n'en avait implémenté que trois, sous le nom `"day"` conflatant deux notions différentes : la *session* de `DEC-04` (le service ouvert) et un *jour calendaire* choisi (passé ou présent). La clarification que le titre de la tâche annonce est exactement ça — `lib/services/metrics.ts::MetricsQuery` devient une union discriminée à cinq branches (`service` | `day` | `month` | `year` | `range`), chacune n'acceptant que les paramètres qui la concernent. `service` reprend le comportement exact que `BI-01` appelait `"day"` (journée ouverte, `expected_cash`/`cash_variance` rattachés) ; `day` est nouveau et répond pour un jour calendaire précis, y compris **après** la clôture du service qui le couvrait, ou pour un jour n'ayant jamais eu de service ouvert — `zonedToday` (`lib/time.ts`) gagne un champ `day` (additif, aucun appelant existant cassé) pour permettre le défaut « aujourd'hui » ; `range` est entièrement nouveau (bornes explicites, aucun défaut honnête n'existant pour une plage à moitié ouverte). `expected_cash`/`cash_variance` ne sont plus jamais rattachés qu'à `service` — une correction directe d'une simplification de `BI-01` qui les attachait à tort à `"day"`. « Aucun filtre visible s'il est ignoré » traduit littéralement en contrat d'API : `lib/validation/schemas.ts::metricsQuerySchema`, une union discriminée de `z.strictObject` (un par période), refuse `?period=year&day=12` avec une erreur nommant le champ `day` comme non reconnu, plutôt que de l'accepter et de l'ignorer silencieusement — un bug d'écran qui enverrait un mois périmé pendant que la période est « année » ne peut plus se produire sans le voir. Aucune route n'appelle encore le service (même choix que `BI-01` : `BI-05` et suivants en seront les consommateurs). **Détour, hors périmètre de cette tâche mais corrigé avec l'accord explicite de l'utilisateur** : en travaillant sur `BI-03`, un test préexistant (`tests/integration/refunds.test.ts`, tâche `ORD-10`) s'est révélé échouer de façon reproductible — vérifié sur le commit `BI-02` déjà committé, avant toute modification. Cause : `lib/repositories/payments.ts::listPaymentsForOrder` triait par `ORDER BY created_at` sans second critère ; pour une vente `MIXED`, les deux lignes `CHARGE` (espèces et carte) sont insérées dans la même transaction, où `now()` est figé — elles portent donc le même horodatage à la milliseconde près, et sans départage l'ordre de retour entre deux lignes à égalité n'est pas garanti par Postgres. `refundOrder` répartit un remboursement « les plus anciennes d'abord » : sans ordre déterministe, un remboursement pouvait toucher la carte avant les espèces de façon imprévisible. Corrigé par l'ajout de `, id` comme départage dans `listPaymentsForOrder`, et le même trou (même cause, même risque réel : plusieurs lignes d'un même mouvement multi-produits insérées dans une seule transaction) corrigé au passage dans `listCashMovements`, `listStockMovements` et `listAuditEvents`. Vérifié par 3 exécutions consécutives du test auparavant instable, toutes vertes. Suite complète revérifiée (`lint`/`typecheck`/`format`/470 tests unit+intégration/75 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-04` — Supprimer tous les historiques fictifs**
  - Priorité : `P0`
  ...
  - Mise en œuvre : audit du fichier avant tout code — **déjà entièrement satisfait**. Les lignes `+150,00 €`/`−20,00 €` codées en dur ont été retirées par `UX-01`/`CASH-07`, et la liste de commandes « 100 dernières, on en affiche 8 » a été remplacée par le vrai historique paginé d'`ORD-12` ; les trois widgets de `app/bilan/page.tsx` (KPI, historique des commandes, journal de caisse) lisent exclusivement `/api/dashboard`, `/api/orders` et `/api/cash-movements`, aucune constante métier n'y subsiste. Ce qui manquait n'était pas du code mais une **preuve depuis l'extérieur du fichier source** : `CASH-09` avait déjà prouvé l'absence de ligne fictive pour le journal de caisse seul, mais rien ne prouvait la même chose pour les cartes KPI et l'historique des commandes, ni le cas le plus direct de tous — un établissement flambant neuf, sans aucune activité, doit afficher un vrai zéro et non un chiffre de démo resté figé. `tests/e2e/bilan-real-data.spec.ts` (nouveau, 2 cas, établissement jetable) : un établissement neuf affiche `0,00 €`/`0`/`0,00 €` sur les trois cartes, aucune carte « Espèces attendues » (aucun service ouvert), et les deux messages d'état vide réels ; puis une vraie vente et un vrai mouvement de caisse se reflètent exactement (`12,00 €` de CA, pas de fuite du mouvement de caisse de `5 €` dans le chiffre d'affaires — preuve que les deux widgets lisent des sources réellement distinctes, pas un même placeholder partagé). **Bug détourné trouvé en écrivant ce test, hors périmètre de `BI-04` mais corrigé dans le même geste** : `tests/e2e/helpers/tenant.ts::dispose()` faisait un simple `DELETE FROM organizations`, qui échouait dès qu'un établissement jetable avait réalisé une vraie vente — aucun spec existant ne combinait jusqu'ici « établissement jetable + vraie vente + suppression en cascade ». Cause : plusieurs clés étrangères sont délibérément **sans cascade** (`order_items.product_id`, `stock_counts.movement_id`/`product_id`, `stock_movements.product_id`, `payments.order_id`) — cohérent avec la philosophie du projet de ne jamais supprimer l'historique par accident (`CFG-02`/`DEC-05`/`DEC-06`). Corrigé côté **aide de test uniquement**, jamais le schéma : `dispose()` supprime désormais explicitement, dans l'ordre que ces contraintes imposent (`stock_counts` → `stock_movements` → `payments` → `orders`), avant de laisser la cascade normale depuis `organizations` nettoyer le reste. Aucune garantie de production affaiblie. Suite complète revérifiée (`lint`/`typecheck`/`format`/470 tests unit+intégration/77 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-05` — Construire le bandeau de contexte**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `components/context-banner.tsx` (nouveau), affiché en tête de `/bilan`, juste au-dessus du sélecteur de période déjà existant — les quatre faits que l'acceptation nomme, toujours ensemble plutôt que laissés à déduire d'un titre de page et de la position courante d'un sélecteur. Aucune nouvelle lecture serveur pour l'établissement et l'état du service : `/api/settings` (`CFG-01`) et `businessDayOpen` de `/api/cash-summary` (`CASH-01`) existaient déjà, réutilisés tels quels — une troisième implémentation de « le service est-il ouvert » aurait été le risque exact que `CASH-04` avait déjà fermé une fois pour les espèces attendues. La période affichée reste dérivée du sélecteur déjà en place (`Aujourd'hui`/`Ce mois`/`Cette année`, branché sur `/api/dashboard`) — **le sélecteur lui-même n'a pas été migré vers les cinq valeurs de `BI-03`** (`service`/`day`/`month`/`year`/`range`) : `BI-03` construit le contrat, aucune tâche jusqu'ici n'a demandé de remplacer un sélecteur de production déjà testé, et `BI-01` avait explicitement laissé ce branchement aux tâches suivantes sans en désigner une précise — chirurgie prématurée hors du périmètre littéral de `BI-05` (« construire le bandeau », pas « refaire le sélecteur »). « Dernière synchronisation » est le seul état réellement nouveau : l'instant où l'un des trois widgets du Bilan a le plus récemment reçu des données fraîches, posé en phase de rendu plutôt que dans un `useEffect` — même schéma que `lib/client/use-async-data.ts` lui-même (une comparaison de signature suivie, pas un `setState` dans un effet, qu'ESLint refuse ici comme source de rendus en cascade). Testé par `tests/e2e/context-banner.spec.ts` (nouveau, 2 cas, établissement jetable) : le bandeau nomme le vrai établissement (pas un texte générique), affiche « Service en cours » par défaut et un horodatage de synchronisation réel une fois les données arrivées ; puis, sans rechargement de page, la pastille bascule sur « Service ouvert » après une ouverture de service faite depuis l'écran caisse, et le libellé de période suit un changement d'onglet vers « Ce mois ». Suite complète revérifiée (`lint`/`typecheck`/`format`/470 tests unit+intégration/79 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-06` — Ajouter les alertes actionnables**
  - Priorité : `P1`
  ...
  - Mise en œuvre : les cinq types viennent de `VISION_PRODUIT_ET_AUDIT.md` §10 (« Bloc À traiter maintenant »), qui nomme exactement ces cinq lignes et fixe leur ordre de priorité — repris tel quel plutôt que réinventé. `lib/services/alerts.ts::getAlerts` (nouveau) — **une ligne par type**, jamais une par entité affectée (trois produits en rupture donnent une alerte « 3 en rupture », pas trois lignes), ce qui rend « maximum cinq » vrai par construction puisqu'il n'existe que cinq types. Aucune détection n'est une nouvelle implémentation : rupture/sous-seuil réutilise `listProducts` (même lecture que `/stock`), écart de caisse compare au seuil de `CFG-00` avec l'exacte même comparaison que `CASH-05` impose déjà à la clôture (`Math.round(seuil * 100)`, centimes entiers), ticket ancien réutilise `listOpenTickets` (`CASH-06`), clôture en retard réutilise `getActiveBusinessDay`. Deux seuils n'existaient nulle part et ont été fixés ici en dur, documentés comme tels plutôt que présentés comme une valeur validée par le métier : ticket ouvert depuis plus de 3 h, service ouvert depuis plus de 16 h — `choisissant mes recommandations`, ajustables plus tard sans changement de schéma si `CFG-00` doit un jour les porter. « Anomalie » (« paiement ou mouvement anormal ») est lu comme un mouvement de caisse resté à la catégorie générique `OTHER` (`DEC-11`) : la catégorisation existe précisément pour que ce cas reste rare, donc son apparition est le signal, plutôt qu'une détection statistique de montant inhabituel, fragile sans historique de référence. « Chacune ouvre l'action correspondante » est honoré au niveau que les écrans actuels permettent réellement : un lien vers l'écran où l'action se fait (`/stock`, `/caisse`, `/bilan`) — aucun de ces écrans n'accepte aujourd'hui un paramètre pour présélectionner un produit, une table ou un ticket précis, ce qui serait un changement d'écran qu'aucune tâche n'a encore demandé ; l'alerte ouvre le bon écran, pas (encore) la ligne exacte. `GET /api/alerts` (nouveau), réservé à `dashboard:view` comme les autres lectures `BI-0x`. Bloc « À traiter maintenant » ajouté sur `/bilan`, juste après le bandeau de contexte (`BI-05`) et avant le sélecteur de période — volontairement indépendant de la période affichée, un ticket bloqué depuis 3 h important que le gérant regarde « Aujourd'hui » ou « Cette année ». Testé par `tests/integration/alerts.test.ts` (13 cas) : chacun des cinq types se déclenche seulement quand sa condition est réellement vraie et reste silencieux sinon, l'ordre de priorité est vérifié avec les cinq déclenchées en même temps, et l'isolation multi-établissement est prouvée ; plus `tests/e2e/alerts-block.spec.ts` (2 cas) — l'état vide honnête pour un établissement sans rien à signaler, et le clic réel sur une alerte de rupture qui ouvre effectivement `/stock`. Suite complète revérifiée (`lint`/`typecheck`/`format`/485 tests unit+intégration/81 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-07` — Ajouter performance et comparaison**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `lib/services/performance.ts` (nouveau) — `getPerformanceComparison`, construit sur les mêmes formules que `BI-01`/`BI-03` (`getRevenueBetween`, `getBusinessDaySummary`), jamais réimplémentées. `DEC-09` fixe une règle de comparaison différente selon la période : pour `mois`/`année`, la période calendaire équivalente précédente (mois précédent, y compris le passage décembre → janvier ; année précédente) ; pour `jour`/`service`, explicitement pas « hier » mais « la moyenne des jours identiques récents (ou le jour identique précédent à défaut d'historique suffisant) », pour ne pas imputer à la performance un simple effet structurel de jour de semaine. Implémenté sans aucune arithmétique de jour de semaine : reculer de 7 jours calendaires à la fois retombe sur le même jour de semaine par construction, donc les quatre occurrences comparées sont simplement `J-7`, `J-14`, `J-21`, `J-28`. Moyenne sur exactement 4 occurrences, **zéros compris** — un jour sans aucune vente reste une vraie mesure (zéro), pas une donnée à exclure de la moyenne ; faire la moyenne d'une seule occurrence *est* le repli « jour précédent » que `DEC-09` nomme, pas un second chemin de code. `service` dérive son jour de référence du **jour calendaire d'ouverture** de la journée active (`business_days.opened_at`, converti dans le fuseau de l'établissement), distinct de `jour` qui répond pour n'importe quelle date choisie, y compris après la clôture du service qui la couvrait — répond honnêtement `« Aucun service ouvert »` sans aucune comparaison quand aucune journée n'est active, plutôt que de comparer un zéro à un zéro. `plage` n'a aucune règle définie par `DEC-09` — extrapolation propre et documentée de cette tâche : la période immédiatement précédente, de même durée. Deux pièges évités structurellement plutôt que par convention : diviser par un `previous` nul (`Infinity`/`NaN`) — `changePercent` vaut `null` dès que `previous` est nul ou absent, un pourcentage contre zéro n'étant pas un nombre réel ; et arrondir la moyenne des commandes à un entier — `ordersCount.previous` reste volontairement fractionnaire (« un lundi typique fait 2,5 commandes »), forme honnête d'une moyenne plutôt qu'un entier fabriqué. Aucune route n'appelle ce service pour l'instant, même choix que `BI-01`/`BI-03` : le livrable de cette tâche est le service et ses tests, pas un écran. Testé par `tests/integration/performance-comparison.test.ts` (nouveau, 8 cas) : moyenne exacte sur les 4 occurrences avec zéros inclus et libellé nommant le bon jour de semaine ; réponse pour un jour explicitement choisi, pas seulement aujourd'hui ; `service` sans journée ouverte puis avec, comparé à une base à zéro (pourcentage `null`) ; mois précédent avec passage d'année ; année précédente ; plage comparée à la fenêtre précédente de même durée ; isolation multi-établissement. Suite complète revérifiée (`lint`/`typecheck`/`format`/493 tests unit+intégration/81 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-08` — Ajouter tendances et ventilation des ventes**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `lib/repositories/trends.ts` (nouveau, cinq requêtes) + `lib/services/trends.ts::getSalesTrends` (les assemble pour une plage donnée). Une seule définition du CA net (`CHARGE` moins `REFUND`, la même que `getRevenueBetween`/`getBusinessDaySummary` de `BI-01`), extraite en CTE partagée (`NET_PER_ORDER`) plutôt que réimplémentée par ventilation. Accepte une plage `{from, to}` brute, pas le contrat à cinq périodes de `BI-03` : cette tâche dépend de `BI-02`/`ORD-12`, pas de `BI-03`, et chaque ventilation répond de la même façon quel que soit le sélecteur de période qui a produit la plage. Les ventes par produit/catégorie comptent les commandes `PAID` **et** `REFUNDED` (même choix que `listSoldItems`, `BI-02`, et pour la même raison : un remboursement partiel est un montant, jamais une liste d'articles, `ORD-10` — aucune ligne ne peut donc être nettée individuellement, et exclure les commandes remboursées ferait disparaître des lignes réellement vendues). « Calcul de rotation limité aux tickets avec table » pris au pied de la lettre : `getTableTurnover` et la durée moyenne de service filtrent tous deux sur `table_id IS NOT NULL`, une vente comptoir n'ayant ni table à faire tourner ni une durée d'occupation qui décrit quoi que ce soit — un test prouve explicitement qu'une vente comptoir à la durée quasi instantanée n'entache ni le nombre de tickets par table ni la moyenne globale. « Chaque valeur mène aux commandes sources » : chaque ligne de chaque ventilation porte sa clé d'identification (heure, jour calendaire, identifiant produit/catégorie, identifiant table) — la donnée nécessaire à un futur drill-down (`BI-11`), qui reste le porteur légitime de son câblage complet vers l'historique. Aucune route n'appelle ce service pour l'instant, même choix que `BI-01`/`BI-03`/`BI-07`. Testé par `tests/integration/sales-trends.test.ts` (nouveau, 4 cas) : répartition heure/jour exacte sur des ventes horodatées avec précision ; ventilation produit/catégorie incluant une commande remboursée et distinguant les produits sans catégorie ; rotation et durée moyenne par table (10 et 20 minutes → moyenne 15) excluant explicitement une vente comptoir quasi instantanée ; isolation multi-établissement. Suite complète revérifiée (`lint`/`typecheck`/`format`/497 tests unit+intégration/81 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-09` — Ajouter le bloc de rapprochement de caisse**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `lib/services/cash-reconciliation.ts::getCashReconciliation` (nouveau) — construit exclusivement sur `getExpectedCash` (`CASH-04`), jamais réimplémenté : c'est littéralement la même fonction que lit la modale de clôture de `CASH-05`, donc les deux ne peuvent pas silencieusement diverger. Trois états, pas un seul. `"open"` (un service est en cours) : les quatre termes lus en direct, `compté`/`écart` valent `null` — rien n'a encore été compté. `"never_opened"` : zéros réels, même principe que `BI-01`/`BI-04` (jamais un chiffre de démo resté figé). `"closed"` (aucun service ouvert, mais un dernier a été clôturé) — le point le plus délicat de la tâche : `attendu`/`compté`/`écart` sont lus **gelés** depuis `business_days` (jamais recalculés), parce qu'un remboursement espèces peut survenir à tout moment sur une commande d'un service déjà clôturé (`ORD-10` ne l'interdit pas), et recalculer `attendu` en direct aurait rendu l'`écart` déjà signé incohérent avec ses propres opérandes — `variance = compté − attendu` doit rester vrai avec les deux valeurs *telles qu'enregistrées*, pas une nouvelle valeur que personne n'a validée. Le détail (fond/ventes espèces/entrées/sorties) reste lu en direct depuis le même registre pour cet état aussi : en fonctionnement normal (aucune mutation après clôture) il somme exactement au total gelé, et le cas où il ne le fait plus plus — un remboursement tardif — est délibéré et documenté, pas un bug caché. Testé par `tests/integration/cash-reconciliation-block.test.ts` (nouveau, 6 cas) : zéro réel avant toute ouverture ; service ouvert dont les quatre termes sont vérifiés terme à terme contre un appel direct à `getExpectedCash` (pas seulement « correct », littéralement le même appel) ; un remboursement espèces qui réduit `ventes espèces` et `attendu` en conséquence (l'acceptation prise au pied de la lettre) ; une clôture dont `attendu`/`compté`/`écart` correspondent exactement à ce qui a été signé ; un remboursement espèces survenant **après** la clôture qui laisse `attendu`/`compté`/`écart` gelés intacts tout en faisant bouger le détail en direct — la garantie de cohérence interne testée explicitement, pas supposée ; isolation multi-établissement. Aucune route n'appelle ce service pour l'instant, même choix que `BI-01`/`BI-03`/`BI-07`/`BI-08`. Suite complète revérifiée (`lint`/`typecheck`/`format`/503 tests unit+intégration/81 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-10` — Ajouter le bloc stock à risque**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `lib/repositories/products.ts::listStockAtRisk` (nouvelle requête, même filtre que `getStockAlertCounts` de `BI-01` — `stock_quantity <= alert_threshold`, produits actifs, triée par quantité croissante) + `lib/services/stock-risk.ts::getStockAtRisk` qui répartit ses lignes en `outOfStock`/`lowStock` selon `stock_quantity === 0`. Une seule requête, deux vues : le total des deux listes ne peut jamais diverger du chiffre déjà compté par `BI-01`, prouvé par un test qui compare directement les longueurs à `getStockAlertCounts`. Contrairement à `BI-01`/`BI-03`/`BI-07`/`BI-08`/`BI-09`, cette tâche sort délibérément du schéma « service et ses tests, pas un écran » : son acceptation, prise au pied de la lettre (« alerte ouvre le produit et son formulaire de mouvement »), est interactive par construction — `lib/services/alerts.ts` (`BI-06`) documentait déjà cette limite dans son propre commentaire (aucun écran n'acceptait de paramètre pour présélectionner un produit), confirmant que `BI-10` est exactement ce travail différé, complémentaire à la carte compacte de `BI-06`, pas une redite. Donc route (`GET /api/stock-risk`, `requirePermission("dashboard:view")`) et composant (`components/stock-risk-block.tsx`) réels. Chaque ligne est elle-même l'action, pas un lien vers une action : le clic ouvre `StockAdjustModal` (`STK-05`) réutilisée telle quelle, `product` pré-rempli et `products={[]}` (vérifié à la lecture que `products` n'est pas utilisé quand `product` est fourni) — enregistrer un réapprovisionnement ici est la même écriture réelle que depuis `/stock`, jamais un raccourci qui contourne le registre. Le bloc se rafraîchit lui-même après un mouvement (`onAdjusted` relance la requête), un produit sorti du seuil disparaissant donc sans rechargement de page. Placement disputé par deux tests e2e préexistants qui présument que leurs propres lignes/cartes sont les seules de la page : `.order-row` (premier = vente de la section « Commandes récentes ») et `.history-card` (dernier = journal de caisse), classes CSS que ce bloc réutilise pour ses propres lignes. Un placement en tête de page cassait le premier ; en fin de page, le second (les deux régressions constatées et corrigées en cours de vérification, pas anticipées). Placé finalement entre les deux sections — le seul emplacement qui ne perturbe ni l'un ni l'autre. Corrigé aussi une lacune CSS : `.movement b.in`/`.movement b.out` (badges existants) sont scopés à un ancêtre `.movement`, absent des lignes `.order-row` de ce bloc — ajout de `.order-row b.in`/`.order-row b.out` dans `app/globals.css`, mêmes couleurs. Testé par `tests/integration/stock-risk.test.ts` (nouveau, 3 cas) : répartition rupture/sous-seuil excluant un produit désactivé et un produit à stock confortable, avec vérification croisée explicite contre `getStockAlertCounts` ; nom de catégorie présent ou `null` ; isolation multi-établissement. Et `tests/e2e/stock-risk-block.spec.ts` (nouveau, 1 cas) : un produit en rupture apparaît sur `/bilan`, le clic ouvre la modale avec le nom et « 0 unités en stock » du bon produit et **sans** sélecteur « Produit » (preuve littérale de la présélection), un mouvement d'entrée de 10 unités fait disparaître le produit du bloc sans rechargement. Suite complète revérifiée (`lint`/`typecheck`/`format`/508 tests unit+intégration/82 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-11` — Implémenter le drill-down et “Voir tout”**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `BI-07`, `BI-08` et `BI-09` avaient chacune délibérément laissé leur propre écran non construit (« le livrable, c'est le service et ses tests, pas un écran ») ; `BI-11` en dépend explicitement les trois, donc la première moitié de cette tâche est de leur construire enfin ce bloc, pas seulement d'ajouter du drill-down à ceux qui existaient déjà. Trois routes minces, chacune validant par `metricsQuerySchema` (`BI-03`, déjà construit pour cette forme exacte) puis transmettant à son service sans logique propre : `GET /api/performance-comparison` → `getPerformanceComparison` ; `GET /api/sales-trends` → `getSalesTrendsForPeriod` (nouveau, `lib/services/trends.ts`) ; `GET /api/cash-reconciliation` (sans paramètre, un seul sujet possible : le service courant ou le dernier clôturé) → `getCashReconciliation`. `getSalesTrendsForPeriod` mérite sa propre note : `getSalesTrends` (`BI-08`) prend un intervalle brut, mais le sélecteur de période du cockpit parle en `MetricsQuery`. Plutôt qu'une troisième traduction « période → intervalle » à côté de celles déjà écrites dans `dashboard.ts` et `metrics.ts`, cette fonction appelle `getMetrics` (`BI-01`/`BI-03`) et relit directement `netRevenue.period` — l'intervalle que les cartes KPI et le bloc de comparaison ont eux-mêmes utilisé pour la même requête, jamais une resynthèse susceptible de diverger d'un jour. `null` (service demandé, aucun n'est ouvert) répond `200`, pas `404` — une période valide sans rien à montrer, pas une erreur. Trois blocs, chacun avec sa propre décision de granularité : `PerformanceComparisonBlock` (BI-07) n'a pas de « Voir tout » — trois chiffres comparés sont déjà tout le détail, rien dessous à ouvrir. `CashReconciliationBlock` (BI-09) non plus, pour la même raison — et son statut `"closed"` comble un vrai trou : avant cette tâche, rien n'affichait plus l'écart figé d'un service une fois clôturé. `SalesTrendsBlock` (BI-08) est la seule vraie navigation : le compact montre le top 5 produits ; « Voir tout » ouvre un `Dialog` (`UX-02`, le même composant que `StockAdjustModal`/`ReceiptDialog`) listant l'intégralité produits/catégories/24 tranches horaires/tables — jamais `daily`, délibérément : une liste de jours n'a pas de borne naturelle (365 lignes possibles pour une année) contrairement aux autres ventilations, bornées par le catalogue ou le nombre de tables ; la donnée reste disponible sur l'API pour un futur graphique. Un `useState` local, jamais une navigation : c'est littéralement ce qui satisfait « retour au dashboard sans perdre le contexte » — il n'y a rien à perdre puisqu'on ne quitte jamais la page. Trois collisions de sélecteurs découvertes et corrigées en cours de route, la même classe de bug que `BI-10` avait déjà révélée une fois : (1) `PerformanceComparisonBlock` reprenait d'abord les libellés « Chiffre d'affaires »/« Commandes »/« Panier moyen » des cartes KPI existantes — `bilan-real-data.spec.ts` cible une carte `.kpi` par ce texte exact sans `.first()`, donc les dupliquer aurait rendu chaque locator ambigu ; renommés en « CA net »/« Ventes »/« Ticket moyen ». (2) `CashReconciliationBlock` affichait « Espèces attendues » inconditionnellement, y compris à l'état `never_opened` — le même test affirme `.kpi[hasText="Espèces attendues"]).toHaveCount(0)` pour un établissement neuf ; renommé en « Solde de caisse attendu ». (3) `SalesTrendsBlock`/`StockRiskBlock` partagent tous deux `.history-card`/`.order-row` avec les sections « Commandes récentes » et « Journal de caisse » (le premier `.order-row` et le dernier `.history-card` de la page sont déjà vérifiés par plusieurs specs e2e) — les deux placés dans le même unique emplacement neutre déjà identifié par `BI-10`, entre les deux. Un audit de tous les `<button>` d'`app/` et `components/` confirme qu'aucun n'est sans `onClick`, sans être un `type="submit"` dans un formulaire, ou sans être un `role="radio"`/`role="tab"` avec son propre gestionnaire — l'acceptation « aucun bouton sans action » tenait déjà avant cette tâche et continue de tenir après. Testé par `tests/integration/sales-trends.test.ts` (3 cas ajoutés) : `null` pour `service` sans journée ouverte ; `service` couvrant exactement depuis l'ouverture jusqu'à maintenant, avec une vraie vente dedans ; `month` résolvant la bonne borne calendaire, excluant explicitement une vente hors mois. Et `tests/e2e/bilan-drilldown.spec.ts` (nouveau, 1 cas) : les trois blocs affichent une donnée réelle issue d'une vraie vente (pas un chiffre figé) ; le bouton « Voir tout » ouvre réellement un drawer contenant ce même produit ; un filtre d'historique posé avant l'ouverture du drawer est toujours actif après sa fermeture. Suite complète revérifiée (`lint`/`typecheck`/`format`/517 tests unit+intégration/83 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-12` — Ajouter l’export CSV**
  - Priorité : `P1`
  ...
  - Mise en œuvre : les quatre domaines validés par `DEC-09` construits sur les quatre fonctions de dépôt que `BI-02` avait déjà écrites et testées pour l'historique JSON paginé (`listSoldItems`, `listPaymentsHistory`, `listCashMovementsHistory`, `listStockMovementsHistory`) — jamais une seconde requête. La seule chose qui leur manquait pour servir un export : répondre « toutes les lignes qui correspondent » quand `limit`/`offset` sont tous les deux absents plutôt qu'une page — un export n'a pas de « page suivante » à demander, et le `limit` des schémas JSON est plafonné à 200, loin d'« un trimestre entier ». Résolu en élargissant les quatre `Filters` (`limit`/`offset` devenus optionnels) et en factorisant la construction du fragment `LIMIT`/`OFFSET` dans `lib/db.ts::buildLimitOffsetClause` (partagée par les quatre plutôt que le même `if (limit !== undefined) { ... }` recopié quatre fois — exactement le genre de quasi-duplication que le principe « une seule formule » de ce projet existe pour éviter, même à cette échelle). Chaque fonction continue de répondre exactement comme avant à tout appelant JSON existant (qui fournit toujours `limit`/`offset` concrets) ; `Page.limit`/`.offset` s'élargissent en `number | null` pour représenter honnêtement l'export (« tout, donc pas de page »), un changement invisible pour les seuls consommateurs actuels de ces types (les quatre routes JSON elles-mêmes, aucun écran ne lit encore ces objets). Format `DEC-09`, appliqué littéralement, pas approximé : `lib/csv.ts::toCsv` (BOM UTF-8 via `String.fromCharCode(0xfeff)` plutôt qu'un caractère littéral invisible dans la source — piégeant à copier/coller sans le voir —, séparateur `;`, fins de ligne `\r\n`, guillemets RFC 4180 uniquement quand un champ en a réellement besoin) et `lib/time.ts::formatZonedIso` (nouveau — l'heure locale de l'établissement avec un décalage numérique explicite, `2026-08-04T19:30:00+02:00`, l'exemple de `DEC-09` lui-même, jamais `toISOString()` qui ne produit que de l'UTC). Les montants ne sont reformatés nulle part : déjà la forme décimale brute exacte que la base et `DEC-09` demandent, transmis tels quels. `lib/services/exports.ts` choisit les colonnes françaises de chacun des quatre domaines et rien de plus — aucun des quatre n'effectue de jointure vers un nom d'utilisateur pour `created_by` (un simple identifiant) : aucune des lignes `BI-02` ne porte de nom, et l'ajouter ici pour la seule commodité de l'export aurait été une seconde définition de « qui a fait ça », honnête sur ce que la donnée est réellement plutôt qu'un nom fabriqué. Quatre routes minces (`GET /api/exports/{sales,payments,cash,stock}`), réservées comme le reste du reporting de la Phase 6 (`dashboard:view`, `OWNER`/`MANAGER`) ; `lib/http.ts::csvOk` (nouveau) pose `Content-Type: text/csv; charset=utf-8` et `Content-Disposition: attachment` pour que le navigateur télécharge le fichier plutôt que d'essayer de l'afficher. `components/exports-section.tsx` : quatre `<a href="/api/exports/...">` ordinaires plutôt qu'un `fetch` + blob — la requête `GET` de même origine porte déjà le cookie de session, et `Content-Disposition` fait le reste ; masqué pour un rôle sans `dashboard:view` (même condition que `canRefund` déjà présente sur cette page) pour qu'une navigation brute n'atterrisse jamais sur un JSON `403` brut à la place de l'état d'erreur habituel de la page. Testé par `tests/unit/csv.test.ts` (5 cas : BOM, séparateur, fins de ligne, guillemetage conditionnel exact RFC 4180, fichier vide toujours préfixé du BOM) ; `tests/unit/format-zoned-iso.test.ts` (5 cas, dont l'exemple de `DEC-09` littéralement recopié en assertion, plus été/hiver/zéro/décalage négatif/minuit) ; `tests/integration/exports.test.ts` (5 cas : les quatre domaines contre une donnée réelle — en-têtes français exacts, montants en décimal brut, dates avec décalage explicite, valeurs métier vérifiées terme à terme — plus l'isolation multi-établissement sur les ventes) ; `tests/e2e/auth.spec.ts` étendu (pas un nouveau fichier, même précédent que `BI-02`) pour les quatre routes d'export refusées à un `CASHIER` avec `403` ; `tests/e2e/exports.spec.ts` (nouveau, 1 cas) : les quatre liens visibles pour un `OWNER`, et le fichier réellement téléchargé décrit la vraie vente qui vient d'avoir lieu (BOM présent, en-tête exact, nom du produit, montant en décimal brut) — jamais une donnée simulée. Suite complète revérifiée (`lint`/`typecheck`/`format`/540 tests unit+intégration/84 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-13` — Tester toutes les métriques**
  - Priorité : `P0`
  ...
  - Mise en œuvre : audit des quatre domaines de l'acceptation avant d'écrire quoi que ce soit, même discipline que `CASH-08` (le précédent de ce projet pour une tâche de réconciliation) avait appliquée à son propre livrable. Trois domaines sur quatre déjà entièrement prouvés ailleurs, délibérément pas redupliqués ici : **caisse** (espèces attendues, écart) — `CASH-08`/`tests/integration/cash-reconciliation.test.ts` reconstruit déjà le tiroir depuis `payments`/`cash_movements`/`business_days` bruts, remboursement, passage de minuit et deux fuseaux inclus ; **stock** — `STK-09`/`tests/integration/stock-ledger-invariant.test.ts` prouve déjà que `products.stock_quantity` égale la somme de son propre registre `stock_movements`, globalement, après une charge mêlant tous les chemins d'écriture, et `BI-01`/`metrics.test.ts` prouve déjà que les compteurs d'alerte se réconcilient avec cette colonne (déjà fiable) ; **formules et périodes dans l'abstrait** — `BI-01`/`BI-03` prouvent déjà que chaque nature de période résout les bonnes bornes SQL et que CA net/commandes/panier moyen se réconcilient avec quelques ventes simples. Ce qui manquait réellement, et que ce fichier couvre : (1) **ventes et paiements**, reconstruits depuis le registre brut à la façon d'un comptable — pas une réassertion de la sortie de `getRevenueBetween`, une seconde formule écrite indépendamment dans le test — à travers espèces/carte, un remboursement total et un partiel dans la même transaction, plus un ticket annulé jamais payé qui ne doit apparaître d'aucun côté ; (2) **remboursements** : `lib/services/metrics.ts::metrics.test.ts` déclarait lui-même différer cet angle jusqu'à `BI-13` — aucun test existant ne plaçait un remboursement devant `getMetrics`. Un remboursement total isolé dans son propre cas : la commande reste comptée (statut `REFUNDED`, jamais `CANCELLED`), seule sa propre contribution au CA net change — le panier moyen se recalcule sur le même nombre de commandes, pas sur les seules commandes non remboursées, ce qu'une implémentation naïve aurait pu faire silencieusement ; (3) **fuseaux**, pour les bornes de période spécifiquement — le propre cas de fuseau de `metrics.test.ts` ne vérifiait que le champ `timezone` restitué, jamais qu'une borne `jour`/`mois` est calculée correctement dans un fuseau loin de l'UTC. Même instant (`2026-01-31T14:00:00Z`), deux établissements : Europe/Paris (CET, UTC+1 en janvier, sans DST) le lit encore en janvier ; Pacific/Auckland (NZDT, UTC+13, été austral) le lit déjà début février — la même preuve que `CASH-08` avait appliquée au tiroir, appliquée ici aux bornes `jour` et `mois`. Testé par `tests/integration/metrics-reconciliation.test.ts` (nouveau, 3 cas, en-tête d'audit explicite citant ce qui est déjà couvert ailleurs) : reconstruction CA net/commandes/panier moyen depuis `payments`/`orders` bruts avec remboursement total et partiel et un ticket annulé exclu ; effet isolé d'un remboursement total sur le panier moyen dérivé ; le même instant absolu atterrissant dans des mois et des jours calendaires différents pour deux établissements dans des fuseaux différents. **Détour trouvé en vérifiant la suite complète, hors périmètre de cette tâche mais corrigé dans le même geste** : `tests/integration/performance-comparison.test.ts` (`BI-07`) échouait de façon intermittente, reproductible en isolation à ce moment précis (et seulement à ce moment). Cause : son propre cas « answers for an explicitly chosen day » extrayait `.getUTCFullYear()/.getUTCMonth()/.getUTCDate()` d'un instant réel, puis passait ces chiffres à une requête interprétée dans le fuseau Europe/Paris de l'établissement — correct sauf durant la fenêtre quotidienne (~22 h–24 h UTC en été, à cause du décalage CEST) où le jour calendaire UTC et le jour calendaire parisien diffèrent, auquel cas le test interrogeait le mauvais jour et ne trouvait pas la vente qu'il venait pourtant de dater à cet instant précis. Exactement l'angle « fuseaux » que cette tâche couvre par ailleurs — corrigé en remplaçant l'extraction UTC brute par `zonedToday("Europe/Paris", instant)` (`lib/time.ts`, déjà utilisée par le code de production lui-même), pour que le test date sa requête dans le même fuseau que celui où il a réellement daté la vente. Suite complète revérifiée (`lint`/`typecheck`/`format`/543 tests unit+intégration/84 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

- [x] **`BI-14` — Tester le cockpit complet**
  - Priorité : `P1`
  ...
  - Mise en œuvre : audit de `GATE-6` (le jalon que cette tâche ferme) poste par poste avant d'écrire quoi que ce soit. Quatre de ses cinq critères déjà satisfaits, chacun par sa propre tâche, jamais redupliqués ici : « aucun contenu mocké » (`BI-04` pour les KPI/commandes/caisse, puis chaque bloc plus récent a prouvé sa propre donnée réelle dans son propre test — `bilan-drilldown.spec.ts` pour comparaison/tendances/rapprochement, `stock-risk-block.spec.ts` pour le stock) ; « chaque KPI possède une définition et un test » (`DEC-09` plus `BI-01`/`BI-13`) ; « les alertes conduisent à une action » (`tests/e2e/alerts-block.spec.ts` : « a rupture alert names the count and its click opens the stock screen » — déjà littéral). Restaient deux trous réels. **« L'export respecte exactement les filtres »** : les quatre liens d'export étaient non filtrés (tout l'historique, quelle que soit la période affichée) — `BI-12` avait livré le format et l'autorisation, jamais le filtrage que `GATE-6` exige explicitement. Corrigé en branchant les quatre routes sur `metricsQuerySchema` (`BI-03`, le même contrat que `/api/performance-comparison`/`/api/sales-trends`) plutôt qu'un `from`/`to` brut ; chaque route résout la période via `resolvePeriodRange` (nouveau, `lib/services/metrics.ts` — extrait de la logique que `getSalesTrendsForPeriod` avait déjà écrite pour `BI-11`, maintenant partagée par les deux plutôt que dupliquée une troisième fois) ; `ExportsSection` reçoit désormais `period`/`year`/`month` comme `PerformanceComparisonBlock`/`SalesTrendsBlock`, donc cliquer « Ventes » pendant que le cockpit affiche « Ce mois » télécharge exactement ce mois. `period: "service"` sans service ouvert répond un fichier valide, l'en-tête seul — un export vide honnête, pas une erreur, le même choix que `getSalesTrendsForPeriod` fait déjà pour le bloc tendances. **« Les détails expliquent chaque total »** : aucun test ne comparait deux widgets différents, lisant le même établissement par deux appels de service différents, sur la même période — chacun n'avait été vérifié que contre son propre calcul. Trois cohérences croisées vérifiées, jamais supposées : le tableau de bord (`getDashboardSummary`), l'enveloppe de métriques (`getMetrics`) et le bloc de comparaison (`getPerformanceComparison`) rapportent le même CA net/commandes/panier moyen pour le même service ouvert ; `CashReconciliationBlock` (`getExpectedCash::cash_sales`) et le tableau de bord (`getBusinessDaySummary::cash_revenue`) — deux requêtes SQL écrites indépendamment pour la même question « CHARGE moins REFUND, espèces, cette journée » — s'accordent au centime près, remboursement inclus ; la somme du détail par produit du bloc tendances égale le CA net du haut de page quand rien n'a été remboursé, et diverge exactement du montant remboursé quand un remboursement existe (prouvé, pas seulement documenté — l'écart entre les deux est lui-même l'assertion, pas un commentaire qui pourrait mentir). Testé par `tests/integration/cockpit-coherence.test.ts` (nouveau, 4 cas) et par le renforcement de `tests/e2e/exports.spec.ts` (le lien « Ventes » porte `period=service` dans son propre `href`, et une période qui ne couvre pas la vente renvoie l'en-tête seul — le filtre est réellement appliqué, pas seulement accepté). Suite complète revérifiée (`lint`/`typecheck`/`format`/547 tests unit+intégration/84 tests e2e sur base fraîche/`build`/`audit --audit-level=high`), toutes vertes.

### `GATE-6` — Pilotage fiable

- [x] Aucun contenu du cockpit n’est mocké.
- [x] Chaque KPI possède une définition et un test.
- [x] Les détails expliquent chaque total.
- [x] Les alertes conduisent à une action.
- [x] L’export respecte exactement les filtres et autorisations.

## 14. Phase 7 — Onboarding, exploitation et sortie MVP

- [x] **`SAAS-01` — Créer l’onboarding initial**
  - Priorité : `P0`
  ...
  - Mise en œuvre : `lib/services/onboarding.ts::createEstablishment` crée les cinq lignes d’un nouveau client — `organizations`, `locations`, `location_settings`, `users`, `memberships` en `OWNER` — dans **une seule transaction**, écrit l’événement d’audit `establishment.create` (`SEC-09`) et ouvre la session dans la foulée. La transaction unique n’est pas décorative : l’organisation et l’établissement sont insérés *avant* l’utilisateur, donc une adresse déjà prise laisserait sinon une organisation orpheline — un client à moitié créé que seule la console SQL supprimée par ce ticket pourrait réparer. `location_settings` est inséré **sans aucune valeur** : toutes les colonnes ont un défaut (`Europe/Paris`, `EUR`, 20 %, seuil 5 € — `migrations/0002`) et les renommer ici forkerait les défauts dans un second endroit susceptible de diverger. Aucune classe de TVA n’est créée non plus : la chaîne de repli de `DEC-05` s’arrête à `location_settings.default_tax_rate`, donc un catalogue est correctement taxé dès le premier jour sans classe. Route publique `POST /api/auth/signup`, volontairement placée sous `/api/auth/` pour hériter du plafond de débit `SEC-07` de `proxy.ts` ; écran `app/signup/page.tsx`, atteignable depuis la page de connexion (jusqu’ici une personne sans compte n’avait aucune porte d’entrée). Contrairement à la connexion et à la réinitialisation de mot de passe, le conflit d’e-mail est **explicite** : un formulaire d’inscription ne peut pas être utile en le masquant, et la personne qui essaie de réserver cette adresse doit savoir qu’elle est prise.
  - Manque comblé au passage : `CFG-02` avait livré le service, l’API et l’audit de création/modification d’un produit, mais **aucun écran ne les appelait** — le commentaire d’en-tête de `app/configuration/page.tsx` renvoyait à « l’écran existant » des produits, c’est-à-dire la page stock, qui ne sait qu’ajuster des quantités (`STK-04`/`STK-05`). Un client pouvait donc créer ses tables mais pas un seul produit : une caisse sans rien à vendre, exactement le `INSERT` manuel que l’acceptation interdit. D’où `components/catalogue-section.tsx` (créer un produit avec prix, catégorie, unité, stock initial et seuil ; retirer et remettre en vente), branché sur la page de configuration. Le stock initial passe par un mouvement `OPENING_BALANCE`, donc l’invariant `DEC-06` tient dès la création. `components/onboarding-checklist.tsx` annonce les trois étapes restantes (tables, produits, premier service) et disparaît de lui-même une fois terminées, plutôt que de devenir du mobilier permanent.
  - Testé par `tests/integration/onboarding.test.ts` (les cinq lignes, les défauts de `location_settings`, la session résolue en `OWNER` sur le bon établissement, l’audit, le conflit d’e-mail insensible à la casse, l’absence de client à moitié créé après ce conflit, l’absence totale d’écriture quand le mot de passe est trop court, et l’isolation de deux inscriptions concurrentes) et par `tests/e2e/onboarding.spec.ts`, seul fichier de la suite **sans aucune fixture** : ni `createThrowawayTenant`, ni `page.request.post`, ni compte seedé — inscription, table, produit, ouverture du premier service et vente de ce produit sur cette table, uniquement au clavier et à la souris. C’est la seule façon de prouver le critère « aucun SQL » tel qu’il est écrit ; le palier intégration atteint `createEstablishment` directement et laisserait la vraie question sans réponse.

- [x] **`SAAS-02` — Gérer l’équipe et les rôles**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `lib/repositories/memberships.ts` + `lib/services/team.ts` + `GET/POST /api/team`, `PATCH /api/team/[userId]` + section « Équipe » sur la page de configuration (`components/team-section.tsx`). Le schéma portait déjà tout le nécessaire (`memberships.role` contraint aux trois rôles, `memberships.status` `ACTIVE`/`DISABLED`) et `users:manage` existait dans la matrice `SEC-05` depuis `DEC-07` — **sans un seul appelant** : aucune route, aucun écran. Aucune migration n’a donc été nécessaire.
  - Ce que le service refuse même au propriétaire : vider l’établissement de ses propriétaires (rétrograder ou désactiver le **dernier propriétaire actif** — personne ne pourrait plus rien restaurer, et la réparation demanderait la console SQL que `SAAS-01` vient de supprimer), et se désactiver soi-même (verrouillage immédiat, récupérable seulement par un autre propriétaire — message distinct, parce que les deux erreurs ne se réparent pas de la même façon). Le compte des propriétaires restants est pris sous `FOR UPDATE` sur **toutes** les lignes propriétaires actives de l’établissement : deux propriétaires se rétrogradant au même instant liraient sinon chacun l’autre comme encore actif et les deux écritures passeraient. Le filtre d’exclusion est appliqué **après** le verrou, en TypeScript, et non en SQL : en `WHERE user_id <> $2` le planificateur le pousse sous `LockRows` (vérifié à l’`EXPLAIN`), chaque transaction ne verrouillait que les *autres* propriétaires — des verrous disjoints, aucune contention, et la course restait entière.
  - Chaque mutation relit sa cible **à travers les adhésions de cet établissement** : l’identifiant vient du client, et c’est ce qui transforme l’identifiant d’un autre locataire en 404 plutôt qu’en ligne modifiable (`SEC-06`/`SEC-08`).
  - Défaut corrigé au passage : `login()` ne vérifiait que `users.status`, le commutateur global du compte, que rien dans le produit ne met à jour. Ce qu’un propriétaire désactive est l’**adhésion** (`memberships.status`) — jusqu’ici cette personne recevait encore un jeton valide, voyait la connexion réussir, puis se faisait éjecter par la première page protégée : un compte qui a l’air de marcher et ne marche pas. Le contrôle est fait après le mot de passe, pour qu’un mot de passe faux et un compte suspendu restent indiscernables (`SEC-03`). Rien ne le remarquait avant ce ticket parce que rien n’écrivait jamais `memberships.status = 'DISABLED'`.
  - Pas d’invitation par e-mail : il n’existe aucun transport de courrier dans ce dépôt (la réinitialisation de mot de passe rend déjà son jeton à l’appelant en développement et se contente de le journaliser en production, cf. `SEC-03` ; le fournisseur réel est un prérequis `OPS-05`). Le propriétaire fixe donc un mot de passe initial qu’il communique de vive voix — dans un bar, il est dans la même pièce que la personne — et l’employé le change par « mot de passe oublié ». Une adresse déjà rattachée à un compte est refusée plutôt que rattachée à l’organisation : `memberships` l’accepterait, mais tirer silencieusement un compte existant dans un établissement donnerait les données de celui-ci à quelqu’un qui n’a rien accepté, sur la seule foi d’un propriétaire ayant deviné une adresse. Le personnel multi-établissements est hors du périmètre MVP de `DEC-01`.
  - Correction remontée en vérification manuelle : les refus fonctionnaient mais **sans message visible**. La page de configuration porte un seul bandeau de notification pour ses cinq sections, tout en haut ; la page fait maintenant ~4000 px, donc un refus levé depuis l’équipe ou le catalogue s’affichait à un millier de pixels au-dessus de la fenêtre et le bouton semblait simplement inerte. Le bandeau est désormais `position: sticky` (`.page-alerts`) plutôt que déplacé : ramener le lecteur en haut de page l’arracherait à la ligne sur laquelle il travaille. Le test e2e ne l’avait pas vu parce que `toBeVisible()` ne dit rien de la position à l’écran — les assertions utilisent maintenant `toBeInViewport()`, vérifié en retirant le `sticky` (deux tests tombent).
  - Testé par `tests/integration/team.test.ts` (15 cas : l’invitation produit un compte réellement utilisable — la personne se connecte et arrive sur le bon locataire avec le bon rôle —, le conflit d’e-mail, l’absence d’écriture sur mot de passe faible, la persistance **relue** d’un changement de rôle, l’audit de `user.invite`/`user.role_change`/`user.disable`/`user.enable`, la suspension qui coupe une session déjà ouverte *et* la reconnexion, les propriétaires suspendus qui ne comptent pas comme derniers propriétaires, la sérialisation réelle de deux rétrogradations concurrentes — pilotée par deux connexions explicites, parce qu’une version antérieure qui lançait simplement deux promesses passait aussi **sans le verrou** —, et l’inaccessibilité des membres d’un autre établissement) et par `tests/e2e/team.spec.ts` (le refus par rôle ne se prouve qu’en HTTP : un responsable ne voit pas la section et reçoit `403` sur `GET /api/team` comme sur `PATCH`).

- [x] **`OPS-02` — Ajouter métriques et alertes d’exploitation**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `lib/observability/` (collecteur + table de seuils), `lib/repositories/operations.ts`, `lib/services/operations.ts`, `GET /api/health/metrics`, et [`docs/exploitation.md`](./docs/exploitation.md) pour le raisonnement. Les six éléments du livrable viennent de deux sources : le processus (disponibilité, latence, erreurs, encaissements échoués — comptés dans `apiRoute`, le seul endroit par lequel toutes les routes passent déjà, pour qu’aucun endpoint ne soit mesuré et un autre silencieusement pas) et la base (clôtures et écarts). Volontairement distinct de `lib/metrics/dictionary.ts`, qui est le registre des KPI **métier** (`BI-01`/`DEC-09`) : les deux vocabulaires restent séparés pour que personne ne lise un p95 comme un chiffre d’affaires.
  - Les seuils sont **codés** (`lib/observability/alerts.ts`) et pas seulement écrits, pour la même raison que la matrice de `DEC-07` vit dans `lib/authz.ts` : un seuil en prose est un seuil que personne n’évalue. Deux destinataires, séparés par qui peut réparer et non par équipe — `astreinte` (le logiciel est cassé, l’établissement n’y peut rien) et `etablissement` (rien n’est cassé, une action métier manque : un service non clôturé, un écart sans motif).
  - « Aucun bruit excessif » repose sur deux règles, vérifiées par mutation. **Aucune alerte sur un refus métier** : `400/401/403/404/409/429` sont le produit qui fonctionne (`SEC-05`, `API-01`, `API-02`, `SEC-07`) ; les compter ferait suivre au taux d’erreur la fréquence à laquelle les gens se trompent de mot de passe, et la première astreinte réveillée pour ça apprendrait à tout le monde à ignorer la suivante — la classification est faite une fois dans `collector.ts::classify`, pas dans chaque appelant. **Aucune alerte sous un volume minimal** : une erreur `500` sur les deux requêtes servies à 6 h du matin fait 50 % de taux d’erreur et n’est pas un incident. Fenêtre base fixée à 24 h — plus courte elle raterait le service clôturé à 2 h, plus longue elle ferait sonner pendant des jours un écart déjà traité.
  - Deux pièges fermés structurellement. **Cardinalité** : la clé de route vient du chemin, qui porte des identifiants (`/api/team/42`) ; sans normalisation en `:id`, la surveillance elle-même fuirait en mémoire, un compartiment par ligne touchée. **État partagé** : le registre est tenu sur `globalThis`, comme le pool de `lib/db.ts` — les route handlers sont bundlés par route, si bien qu’une `Map` de module donnait à chaque endpoint son registre privé et `/api/health/metrics` ne rapportait que ses propres requêtes. C’est exactement ce qui s’est produit à la première exécution (`requests: 0` sur un serveur qui venait d’en servir une dizaine), et une sonde qui sous-compte est pire qu’absente : elle se lit « tout va bien ».
  - Accès par secret d’opérateur (`OPS_METRICS_TOKEN`, comparaison à temps constant) et non par rôle : l’endpoint rapporte sur **tous les établissements à la fois**, et aucun rôle de `DEC-07` n’accorde ça — un propriétaire administre son établissement, pas la plateforme. Sans le jeton configuré, il refuse au lieu de s’ouvrir. Aucun transport d’alerte n’est inventé (il n’en existe aucun dans ce dépôt ; prérequis `OPS-05`) : les alertes sont rendues déjà qualifiées — gravité, destinataire, valeur observée, seuil franchi — pour qu’un collecteur externe n’ait plus qu’à les router.
  - Détail corrigé en vérification manuelle : `StaleOpenBusinessDayRow.opened_at` était typé `string` alors que node-postgres renvoie un `Date`, si bien que l’alerte interpolait « Wed Sep 02 2026 22:13:24 GMT+0200 (Central European Summer Time) » — inexploitable par un collecteur, et différent d’une machine à l’autre. Type rendu honnête, instant émis en ISO 8601 UTC, garde ajoutée au palier intégration.
  - Testé par `tests/unit/observability.test.ts` (18 cas : classification, cardinalité, centiles — dont le fait qu’une seule requête lente sur vingt ne déplace pas le p95, ce qui est la raison même de mesurer un centile plutôt qu’un maximum —, silence sous le volume minimal, déclenchement au-dessus, et une instance chargée et saine qui ne dit rien), `tests/integration/operations.test.ts` (9 cas : service ouvert trop longtemps vs. simple passage de minuit autorisé par `DEC-04`, écart hors seuil sans motif, silence quand un motif existe, **seuil propre à chaque établissement** — le même écart de 30 € alerte à 5 € de seuil et pas à 50 € —, sortie de fenêtre) et `tests/e2e/operations-metrics.spec.ts` (5 cas : le jeton, le refus d’une session de propriétaire, la présence des six éléments du livrable, et le fait que les refus de l’endpoint lui-même ne déclenchent aucune alerte).

- [x] **`OPS-03` — Mettre en place sauvegarde et restauration**
  - Priorité : `P0`
  ...
  - Mise en œuvre : `scripts/backup.mjs`, `scripts/restore.mjs`, socle commun `scripts/lib/backup-core.mjs`, commandes `pnpm db:backup` / `pnpm db:restore`, et [`docs/sauvegarde-restauration.md`](./docs/sauvegarde-restauration.md) pour la procédure. Dump `pg_dump` custom (`-Fc`, compressé, `--no-owner --no-acl` pour qu'il se restaure sur un serveur où les rôles de production n'existent pas — une sauvegarde qui ne se restaure que sur sa propre machine n'en est pas une), accompagné d'un **manifeste** : instant de prise, base, version de schéma, taille, empreinte SHA-256, durée. La version de schéma est enregistrée parce que restaurer une sauvegarde antérieure à une migration dans une application qui attend le schéma plus récent est un échec qu'il vaut mieux prévoir que découvrir à 3 h du matin. Rétention `DEC-10` (30 jours glissants puis une hebdomadaire sur 3 mois) appliquée **après** l'écriture de la nouvelle sauvegarde, pour qu'un dump raté ne supprime jamais rien.
  - Trois refus, codés dans le script et pas seulement écrits dans la procédure. **Jamais par-dessus la base applicative** (`DEC-10` : « jamais directement en production ») : la faute que cela empêche est un drapeau oublié. **Empreinte vérifiée avant toute destruction** : un dump corrompu découvert *après* avoir vidé la cible ne laisse plus rien du tout. **`--single-transaction`** : la restauration arrive entière ou laisse une base vide, une base à moitié restaurée qui répond aux requêtes étant le résultat le plus facile à confondre avec une base saine. `pg_dump` est cherché sur le `PATH` puis dans le conteneur Docker, et le repli conteneur est refusé si `DATABASE_URL` ne pointe pas vers une base locale — sinon sauvegarder une base managée passerait par le conteneur local et rendrait un dump de la mauvaise base.
  - « Automatique » rendu **vérifiable** plutôt que supposé : la fraîcheur de la dernière sauvegarde alimente deux alertes critiques d'`OPS-02` (`backup_missing`, `backup_overdue` au-delà de 26 h — 24 h de RPO plus une marge pour qu'un travail nocturne décalé ne réveille personne chaque nuit). Les deux sont distinctes à dessein : une planification qui n'a jamais tourné n'est pas le même problème qu'une planification arrêtée. L'âge est lu dans le **manifeste**, jamais dans la date du fichier — un dump peut être copié ou synchronisé longtemps après sa prise, et un RPO calculé sur « quand ce fichier a bougé » annoncerait une sauvegarde fraîche sur une planification arrêtée depuis des semaines. Une planification que personne ne vérifie se découvre cassée le jour où l'on en a besoin, le seul jour où elle ne peut plus être réparée.
  - Testé par `tests/integration/backup-restore.test.ts` — l'exercice complet à chaque exécution de la suite, ce qui **est** le critère d'acceptation : données distinctives écrites, `pg_dump` réel, restauration dans une base créée pour l'occasion, puis relecture (organisations, empreintes de mots de passe intactes — un compte restauré sans ses identifiants est un établissement qui ne peut pas se connecter le pire jour de son année —, événement d'audit, contraintes de clés étrangères, table des migrations), plus les deux refus, vérifiés par mutation. `tests/unit/backup-retention.test.ts` couvre la rétention à part, parce qu'un bug de rétention est le seul qui se cache : il supprime en silence et le résultat est indiscernable de l'absence de sauvegardes — d'où le choix de garder toute sauvegarde que le script ne sait pas dater.
  - Hors périmètre, renvoyé à `OPS-05` : chiffrement au repos et stockage hors machine (`DEC-10`), restriction d'accès à la base de production, et le reste du RTO de 4 h ouvrées — le script ne couvre que l'étape base, qu'il chronomètre et affiche explicitement comme telle pour que personne ne lise « 0,4 s » comme un RTO tenu.

- [x] **`OPS-04` — Implémenter rétention et suppression**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `scripts/lib/retention-core.mjs` + `scripts/data-request.mjs` (`pnpm data:request status|export|anonymize|purge`), documenté dans [`docs/conservation-suppression.md`](./docs/conservation-suppression.md). Outil en ligne de commande et non écran : ce sont des actes rares et irréversibles réalisés par le responsable d'exploitation désigné (`OPS-09`), pas par le propriétaire d'un établissement depuis une page qu'il pourrait atteindre par mégarde ; et les placer derrière un endpoint HTTP aurait signifié un jeton capable d'effacer un établissement, ce qui est pire à posséder qu'un script qu'il faut délibérément lancer. Même précédent que `seed.mjs`/`migrate.mjs` : le garde `SEC-06` « aucune requête directe » porte sur les *route handlers*.
  - Le ticket tient entier sur un fait de schéma : **une ligne `users` ne peut pas être supprimée** dès que la personne a agi — sept tables la référencent en `NO ACTION` (`orders`, `payments`, `cash_movements`, `stock_movements`, `stock_counts`, `business_days`, `audit_events`), parce que `DEC-10` conserve les écritures six ans et qu'aucune ne veut rien dire sans l'identité de qui l'a enregistrée. Une demande de suppression est donc honorée par **anonymisation sur place** : la ligne et son identifiant survivent, chaque écriture reste rattachée à un acteur stable, et la personne cesse d'être identifiable. C'est ce qui permet aux deux exigences de `DEC-10` d'être vraies en même temps, et c'est littéralement le critère d'acceptation.
  - Détails qui comptent : l'adresse de remplacement (`anonymise-<id>@supprime.invalid`) satisfait l'index d'unicité et ne peut jamais recevoir de courrier (`.invalid` est réservé par la RFC 2606 pour cela) ; le mot de passe devient une valeur qui n'est le haché de rien, donc aucun mot de passe ne peut plus correspondre ; l'ancienne adresse n'est **pas** consignée dans l'audit — l'y écrire conserverait l'identifiant même que la demande visait à effacer, dans une table où rien n'a le droit de supprimer. L'opération est auditée sur chaque établissement où la personne était membre (`SEC-09`). La purge, elle, n'est pas auditée en base : `audit_events` est rattaché à l'établissement et vient d'être supprimé avec lui ; l'archive prise avant et le compte rendu du script sont la trace.
  - `status` compte table par table les écritures encore dans la fenêtre de six ans et annonce **à partir de quelle date** une purge devient possible, pour qu'un opérateur puisse répondre « le 4 septembre 2032 » plutôt que « plus tard ». `purge` refuse tant qu'une seule écriture y reste, et refuse aussi sans confirmation explicite (`DEC-10`) ; la suppression se fait enfants d'abord, ce schéma ne cascadant que dans un sens. `export` rend des lignes brutes et non les CSV du cockpit (`BI-12`) : ceux-là répondent à des questions de gestion et omettent ce dont ils n'ont pas besoin, celui-ci existe pour que le client garde ce que la suppression va retirer, et une omission y serait irrattrapable.
  - Testé par `tests/integration/retention.test.ts` (15 cas). Le plus important : après anonymisation d'un caissier ayant encaissé, la vente est toujours là, toujours attribuée, toujours jointe — total inchangé, paiements et mouvements intacts. Vérifié par mutation : remplacer l'anonymisation par une suppression fait tomber ce test, tout comme retirer le contrôle de conservation avant la purge ou consigner l'ancienne adresse dans l'audit. Couvre aussi le verrouillage immédiat du compte (session ouverte et reconnexion), l'atomicité (une interruption laisse le compte intact plutôt qu'à moitié effacé — pire qu'un compte non traité), l'isolation entre établissements pour l'export comme pour la purge, et le refus d'anonymiser deux fois.
  - Volontairement absent : aucune interface et aucun déclenchement automatique à J+30. `DEC-10` fixe un délai de traitement, pas une échéance à automatiser ; la demande arrive par un canal humain. Automatiser une suppression irréversible sur minuterie créerait un risque plus grand que celui qu'elle retirerait.

- [x] **`OPS-05` — Préparer le déploiement de production**
  - Priorité : `P0`
  ...
  - Mise en œuvre : `Dockerfile` (trois étapes, sortie `standalone`, utilisateur non privilégié, ~293 Mo), `docker-compose.prod.yml`, `.env.production.example`, `scripts/check-env.mjs`, `lib/schema-version.ts`, et [`docs/deploiement.md`](./docs/deploiement.md). Les migrations **et leur exécuteur voyagent dans l'image** : le schéma qu'une version attend fait partie de cette version. Le service `migrate` est séparé de l'entrée applicative — plusieurs répliques se disputeraient sinon la même base, et une migration en échec doit arrêter le déploiement plutôt que laisser un conteneur boucler alors que la version précédente est déjà partie. Postgres n'est pas publié sur l'hôte, l'application l'est sur `127.0.0.1` seulement : le reverse proxy est devant, et `DEC-02` y termine TLS plutôt qu'ici — mettre des certificats dans cette pile dupliquerait la responsabilité à deux endroits, ce qui est la manière dont l'un des deux finit expiré.
  - « Aucune variable de développement » tenu par un refus de démarrer, pas par une consigne : `scripts/check-env.mjs` rejette `NODE_ENV` non-production, une base sur la boucle locale, le mot de passe `kalloud_dev_password` de `docker-compose.yml`, un `OPS_METRICS_TOKEN` absent/d'exemple/trop court, les trois échappatoires locales, et `AUTH_RATE_LIMIT_MAX=1000` — le plafond de la suite de tests, qui en production n'est pas une limite de débit mais une porte ouverte avec un panneau dessus. Chaque valeur que ce dépôt livre pour le confort local **fonctionne** : c'est précisément pourquoi elle arrive en production sans que rien ne casse. Tous les problèmes sont rapportés d'un coup, un opérateur voulant la liste complète et pas un élément par redémarrage.
  - « Rollback testé » : les migrations sont en avant seulement, donc revenir à l'image précédente laisse la version antérieure face à un schéma plus récent — ce qui *semble* marcher jusqu'à la requête touchant une colonne renommée, en plein service. `/api/health/ready` compare donc le schéma de la base aux migrations de l'image et **distingue les deux sens** : migrations manquantes = déploiement inachevé, migrations inconnues = rollback. Vérifié en conditions réelles sur la pile lancée : `503` dans les deux cas, conteneur marqué `unhealthy` par Docker (donc plus de trafic routé) pendant que `/api/health/live` reste vert (donc pas de redémarrage en boucle d'un processus sain), puis retour au vert.
  - Deux défauts réels trouvés en construisant vraiment l'image. **Le build exigeait `DATABASE_URL`** : le pool était créé à l'import du module, et `next build` importe chaque route pour collecter les données de page — la construction échouait donc sur toute machine sans `.env`, y compris un conteneur propre ; la CI et le poste local le masquaient en écrivant un `.env` d'abord. C'est le mauvais couplage pour un déploiement : la machine de construction n'a pas à détenir le mot de passe de la base, et l'artefact ne doit pas dépendre de la base joignable ce jour-là. Le pool est désormais créé à la première utilisation (`Proxy`, appelants inchangés, message d'erreur conservé). **Le fichier de verrouillage déclarait un `overrides` absent de `package.json`** (l'`override` `nanoid`), toléré par pnpm 10 et refusé par pnpm 11 — que `corepack enable` récupérait faute d'épinglage. Version fixée dans `packageManager`, `overrides` déclaré : `pnpm install --frozen-lockfile` passe désormais quelle que soit la version.
  - Corrigé aussi en le lançant : la porte de configuration refusait d'abord le déploiement qu'elle est censée protéger, en rejetant l'hôte `postgres` — un service compose atteint par un réseau privé est une topologie de production ordinaire, et la liste de `pg-wait.mjs` (qui garde des scripts destructifs, où « un conteneur sur cette machine » est justement le cas à surveiller) n'était pas la bonne ici. Seule la boucle locale reste refusée.
  - Troisième défaut trouvé en faisant tourner la suite : le `Proxy` du pool ne transmettait que `get`/`set`, si bien que `vi.spyOn(pool, "query")` s'installait sur la cible vide pendant que les lectures venaient du vrai pool — le test d'`API-01` qui prouve « avant l'accès base » en comptant les requêtes observait donc **zéro requête** pour un encaissement qui avait bien tourné. Tous les pièges transmettent désormais, y compris ceux des descripteurs de propriété. C'est la suite existante qui l'a attrapé immédiatement.
  - Testé par `tests/unit/env-check.test.ts` (12 cas, écrits contre les chaînes littérales que ce dépôt contient réellement — dont un cas isolant la règle « valeur d'exemple » avec un jeton assez long, parce qu'une première version testait des jetons courts que la règle de longueur attrapait déjà, si bien que la règle visée n'était pas exercée du tout) et `tests/integration/schema-version.test.ts` (4 cas : concordance, base en avance, image en avance, et les deux à la fois). Vérifié par mutation.
  - Hors de portée sans hébergeur, et dit comme tel : certificat TLS et nom de domaine réels, le reverse proxy lui-même, le stockage de sauvegardes hors machine et son chiffrement au repos (`DEC-10`), et le rollback de bout en bout sur une plateforme. La mécanique qui rend ce rollback sûr est testée ici ; le déploiement attend une plateforme.

- [x] **`OPS-06` — Vérifier les parcours complets**
  - Priorité : `P0`
  ...
  - Mise en œuvre : `tests/e2e/parcours-complet.spec.ts` — les six étapes de l'acceptation dans l'ordre, en un seul parcours, sur un établissement qui n'existait pas au démarrage du test. Deux choses le distinguent du reste de la suite plutôt que d'en faire une version plus longue. **Aucune fixture** : l'établissement est créé par le formulaire d'inscription (`SAAS-01`), donc rien de seedé n'est lu ni supposé — et ce n'est pas une affirmation, `pnpm test:e2e:fresh` réinitialise et migre la base **sans la semer** puis lance ce seul fichier ; il passe avec zéro organisation, zéro utilisateur et zéro produit en base. **La base est la dernière assertion, pas l'écran** : chaque étape est confrontée aux tables brutes, et la caisse attendue à la clôture est **recalculée depuis le registre** — comme le ferait un comptable — puis comparée au montant que l'application a figé. Un écran peut s'accorder avec une API qui s'accorde avec un service, tous partageant la même hypothèse fausse ; le registre, non.
  - Invariants réconciliés en base à chaque étape : l'établissement neuf ne possède rien et porte déjà le taux de repli de `DEC-05` (sans lui la toute première vente serait intaxable) ; `products.stock_quantity == SUM(stock_movements.quantity)` dès la création du produit, le stock initial étant un mouvement `OPENING_BALANCE` traçable et non un nombre écrit dans une colonne (`DEC-06`) ; le fond d'ouverture inscrit à la fois sur la journée et au journal (`CASH-01`/`CASH-03`) ; `orders.total_amount` égal à la somme de ses propres lignes et non à un chiffre envoyé par le client (`SALE-06`) ; le ticket qui survit à un rechargement complet (`ORD-04`) ; le paiement égal à ce qui a été facturé ; la vente inscrite comme mouvement négatif plutôt que comme réécriture silencieuse ; la formule `CASH-04` recalculée ; et la trace d'audit (`SEC-09`).
  - Vérifié par mutation, sur deux invariants que ce parcours est seul à tenir de bout en bout : une vente qui ne retire qu'une unité au lieu de deux fait tomber la réconciliation du stock ; une caisse attendue qui ignore les ventes espèces — le défaut d'origine de `CASH-04` — fait tomber la clôture, et la sortie d'échec montre exactement la forme du bug, un écran qui se contredit lui-même (« Ventes espèces 8,00 € » au-dessus d'« Espèces attendues 100,00 € »).
  - Une hypothèse fausse corrigée en écrivant le test : la période « Aujourd'hui » du cockpit désigne le **service en cours** (`DEC-04`/`BI-03`), pas le jour calendaire, et renvoie donc légitimement zéro juste après une clôture. Le parcours lit le mois pour poser la question qui l'intéresse — la vente a-t-elle atteint le cockpit — et vérifie au passage le zéro d'« Aujourd'hui », qui est le comportement voulu.

- [x] **`OPS-06B` — Vérifier tous les parcours P1 du MVP**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `tests/e2e/parcours-par-role.spec.ts`. L'audit préalable a montré le trou exact : la suite comptait **21 connexions propriétaire contre deux pour chacun des autres rôles**, et ces deux-là ne vérifiaient que des refus. Le dépôt avait longuement prouvé qu'un rôle est *refusé* ce que `DEC-07` lui interdit — `tests/unit/authz.test.ts` teste la matrice ligne à ligne, une demi-douzaine de specs vérifient le `403`. Ce qu'aucun test ne demandait, c'est l'inverse : un caissier peut-il réellement tenir un service ? un responsable peut-il faire tourner l'établissement ? `createThrowawayTenant` a donc gagné `addMember(role, label)`, sans quoi il n'y avait aucun moyen de *travailler* comme responsable ou caissier plutôt que de se faire éconduire comme tel.
  - Chaque rôle fait une vraie journée, **en séquence sur un même établissement**, ce qui exerce au passage les passages de relais d'un service réel — le caissier ouvre et encaisse, le responsable corrige le stock, rembourse, lit le cockpit et exporte, le propriétaire configure, le caissier clôture. Des exécutions isolées par rôle auraient précisément manqué ça. La clôture réconcilie la journée des trois : 60 € de fond, +10 € de vente espèces (caissier), +5 € d'appoint (caissier), −10 € remboursés en espèces (responsable) = 65 €. `describe.serial`, la journée de caisse étant un état d'établissement — même raison que `business-day-open-close.spec.ts`.
  - Les quatre volets du livrable : **opérations** (ouverture, ticket, encaissement, mouvement de caisse, ajustement de stock, remboursement, clôture), **interfaces** (la navigation est filtrée par permission, donc la liste des liens est la lecture la moins chère de « ce que cette personne a le droit de faire » : `[Caisse, Stock]` pour un caissier, les quatre pour les deux autres ; réglages en lecture seule pour un responsable, section Équipe absente), **drill-down** (cockpit et onglet « encaissées »), **exports** (le CSV réel, pas le lien : `period=service`, ce que porte le lien du cockpit sur l'onglet « Aujourd'hui »).
  - Deux locators faux corrigés en route, dont un qui **passait pour rien** : la vérification « un caissier ne peut pas ajuster le stock » cherchait un bouton nommé `/ajuster/i` qui n'existe nulle part dans cet écran, donc elle ne vérifiait rien. Elle porte désormais sur le nom accessible réel (`Ajouter du stock pour …`) et, plus largement, sur le fait que la ligne de stock d'un caissier n'offre **aucun** bouton.
  - Défaut remonté en vérification manuelle : côté responsable, les réglages étaient bien inertes mais **rien ne le disait à l'œil** — la feuille de style ne portait aucune règle `.input:disabled`, et `.input` impose son propre fond blanc et sa couleur d'encre, ce qui écrase le rendu désactivé du navigateur. Le responsable cliquait dans un champ d'apparence ordinaire et il ne se passait rien ; la phrase sous la carte l'expliquait, mais seulement après qu'il ait essayé. Même famille que le bandeau invisible de `SAAS-02` : correct dans le DOM, faux pour l'humain. Fond, bordure, couleur et curseur distincts désormais, le texte restant à `--muted` (≈ 6:1) plutôt que délavé jusqu'à l'illisible — ces valeurs sont une information dont un responsable a besoin, elles ne sont simplement pas à lui de les changer. La garde ne se contente plus de `toBeDisabled()` — c'est exactement ce qui avait laissé passer le défaut — mais compare le rendu calculé à celui d'un champ réellement modifiable.
  - Second défaut, trouvé parce que le test était **instable** — une exécution complète de la suite sur cinq échouait sur la lecture de la navigation du caissier, qui y voyait « Bilan » et « Réglages ». Ce n'était pas un artefact de test : pendant que la session se résout, `role` est `undefined`, et le filtre retombait sur « tout afficher ». Un caissier voyait donc brièvement des liens qui disparaissaient ensuite — et un lien sur le point de disparaître est un lien qu'on peut cliquer. Ne pas connaître le rôle n'est pas une raison d'en offrir davantage : les liens soumis à permission restent masqués tant que la réponse n'est pas là. Le test, lui, attend désormais que l'identité s'affiche dans la barre du haut avant de lire le menu, ce qui est le moment où la session a répondu. Cinq exécutions complètes consécutives au vert après correction.
  - Vérifié par mutation sur la matrice elle-même : accorder `dashboard:view`/`export:create` à un caissier fait tomber la lecture de sa navigation ; accorder `settings:manage` à un responsable fait tomber le refus côté propriétaire.

- [x] **`OPS-07` — Faire la recette accessibilité et responsive**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `tests/e2e/helpers/a11y.ts` + `tests/e2e/accessibilite.spec.ts`, rapport dans [`docs/accessibilite.md`](./docs/accessibilite.md). Une recette ne vaut que si elle se rejoue, donc la moitié mesurable est un test et non un compte rendu : contraste **rendu** de chaque nœud de texte calculé contre la couleur réellement peinte derrière lui, débordement horizontal aux cinq largeurs de `UX-04`, taille de cible, nom accessible de chaque contrôle, lien d’évitement, focus visible, piège de focus/Échap/restitution du focus, et une vente complète au clavier seul. Écrit à la main plutôt que tiré d’une bibliothèque : les deux questions qui comptent ici — le contraste rendu et la taille des cibles sur une caisse utilisée debout — se calculent exactement, et un outil générique aurait noyé ces deux réponses sous cent constats.
  - Trouvé et corrigé : **cinq gris codés en dur que la correction de `UX-04` n’a jamais atteints** — le token `--muted` avait bien été assombri (`#718078` → `#526058`) et documenté, mais ces règles portaient leur propre copie de l’ancienne valeur, donc rien ne leur est parvenu : navigation (3,06:1), onglets de période (3,98:1), légendes de KPI (4,11:1), lignes de ventilation (4,15:1), plus une carte. **`.split b` peignait `--ink` sur la carte vert foncé : 1,35:1**, les montants de la ventilation de caisse pratiquement invisibles — seul `.kpi-label` avait reçu une variante claire. **`.pill.busy` à 4,04:1**, le badge « table occupée », trouvé seulement au second passage parce que le premier avait tourné sur un plan de salle entièrement libre, qui n’est l’écran de personne ; le test crée désormais cet état au lieu de l’attendre. **Quatre cibles sous le minimum**, dont les onglets de filtre du bilan à 23 px de haut — sous les 24×24 de WCAG 2.2 AA — parce qu’ils vivaient dans un conteneur `.segmented` **sans porter la classe `.segment`** : le même composant implémenté deux fois, dont la moitié n’avait jamais reçu le dimensionnement ; partager la définition unique était la correction, une seconde règle CSS aurait recréé la divergence.
  - Deux fausses pistes écartées plutôt que « corrigées » : `NEXTJS-PORTAL` (l’hôte de la surcouche d’erreur du serveur de développement, absent d’une version de production) et `body` pendant le bouclage du focus à travers la barre du navigateur. Une version antérieure de ces tests échouait sur les deux et m’aurait fait corriger un piège de focus qui fonctionne parfaitement.
  - Le contraste et le nommage sont audités à **deux largeurs** et non cinq : la couleur ne dépend pas de la largeur, seule la mise en page autour d’elle change, et cette application en a deux. Les auditer toutes répétait quatre fois les mêmes constats et coûtait assez de chargements pour que le serveur de développement lâche des connexions ailleurs dans la suite (`ECONNRESET` dans une spec voisine, environ une exécution sur trois). Le débordement, lui, dépend de la largeur et reste audité aux cinq. Quatre suites complètes consécutives au vert après ce réglage.
  - **Non couvert, et dit comme tel** : un lecteur d’écran réel (la suite vérifie que chaque contrôle *a* un nom, pas que l’enchaînement annoncé est compréhensible), le jugement visuel sur un vrai téléphone en plein service, le zoom à 200 % (`1.4.4`) et l’espacement de texte (`1.4.12`), et `prefers-reduced-motion`. Ces quatre points relèvent d’un œil humain et sont listés comme tels dans le rapport.

- [x] **`OPS-08` — Faire la revue de sécurité finale**
  - Priorité : `P0`
  ...
  - Résultat : **aucun finding critique ni élevé**. Trois findings moyens ou faibles, tous corrigés et tous couverts par un test qui les empêche de revenir. Rapport complet dans [`docs/revue-securite.md`](./docs/revue-securite.md).
  - Ce qui tenait déjà : mots de passe `bcrypt` ; jetons de session de 32 octets aléatoires **stockés hachés**, avec expiration, révocation et vérification du statut du compte *et* de l'adhésion ; comparaison à un haché factice pour qu'une adresse inconnue ne se trahisse pas par le temps de réponse ; limitation sur les **échecs** seulement, ce qui évite de bloquer un poste qui travaille ; cookies `httpOnly`/`sameSite`/`secure` en production. **Aucun chemin d'injection** : les cinq constructeurs de clause `where` poussent chaque valeur dans un tableau de paramètres et n'émettent que des `$n`. Erreurs inattendues journalisées intégralement côté serveur et répondues par un message générique, sans trace d'appel ni fragment SQL. `pnpm audit --audit-level=high` propre.
  - **Finding 1 (moyen)** — `GET /api/orders` n'avait aucun garde : un caissier obtenait l'historique des commandes payées de l'établissement, montants, statuts et **nom de qui a encaissé**, filtrable par période et paginé. `DEC-07` dit qu'un caissier voit sa session en cours « mais pas l'historique complet » ; l'écran Bilan lui est masqué et `/api/dashboard` lui répond `403` — cet endpoint était la seule porte restée ouverte sur les mêmes données, alors que ses frères `/api/sales` et `/api/payments` portaient déjà `dashboard:view`. Confirmé en conditions réelles avant correction : une vente encaissée par le propriétaire, relue par le caissier.
  - **Finding 2 (faible)** — `GET /api/settings` n'avait aucun garde et expose `cashDiscrepancyThreshold`, le montant sous lequel un écart de caisse ne demande **aucun motif écrit** (`CASH-05`), c'est-à-dire précisément le chiffre que voudrait connaître quelqu'un qui rogne le tiroir. Gardé sur `tables:manage` et non `settings:manage` : c'est la lecture, et c'est déjà la permission avec laquelle la navigation décide qui peut ouvrir l'écran de configuration.
  - **Finding 3 (faible)** — la rédaction des journaux ne masquait qu'au premier niveau. Aucune fuite active, chaque appel journalisant des scalaires, mais `LogFields` est un `Record<string, unknown>` et le premier `logger.info("…", { body })` écrit par quelqu'un aurait imprimé un mot de passe en clair, dans le module dont le contrat est l'inverse (`OPS-01`, `DEC-10`). Un garde qui tient tant que tout le monde s'en souvient n'est pas un garde : rédaction rendue récursive, plafonnée en profondeur pour qu'un objet cyclique ne fasse pas boucler une ligne.
  - Les deux findings d'autorisation sont des **lectures**, et c'est l'enseignement de la revue : une matrice de permissions s'applique naturellement aux écritures, qui sont visiblement dangereuses, et s'oublie sur un `GET` qui « ne fait qu'afficher ». Le test correspondant vérifie donc les deux sens — le caissier reçoit `403` sur ce qui ne le regarde pas, **et** `200` sur tout ce que `DEC-07` lui laisse, sans quoi la correction casserait son service.
  - Testé par `tests/e2e/revue-securite.spec.ts` (5 cas), `tests/unit/logger-redaction.test.ts` (6 cas) et un contrôle statique ajouté à `tests/unit/architecture.test.ts` : les interpolations SQL existantes sont énumérées et une nouvelle fait échouer le test — liste blanche à dessein, une interpolation dangereuse ressemblant exactement aux interpolations sûres qui l'entourent. Vérifié par mutation en insérant `${filters.status}` dans une requête réelle.
  - Observation reportée à `CLEAN-01` : `lib/client/use-currency.ts` (`useCurrencyFormatter`) n'est importé nulle part — code mort, pas un problème de sécurité.

- [x] **`OPS-08B` — Revoir la sécurité des parcours P1**
  - Priorité : `P1`
  ...
  - Résultat : **aucun finding**, critique, élevé ou autre. Ce qui a été trouvé n'est pas une faille mais un **angle mort de la couverture**, et c'est lui qui est refermé. Rapport dans [`docs/revue-securite.md`](./docs/revue-securite.md).
  - L'angle mort : la spec d'isolation navigateur de `SEC-08` couvre exactement **un** endpoint prenant un identifiant (`/api/products/[id]`) ; le produit en compte **douze** — reçu, remboursement, comptages de stock, les quatre routes de ticket, catégories, tables, équipe. Onze sont arrivés après la signature du socle, et aucun n'avait jamais reçu la seule question qui compte pour eux : que se passe-t-il quand l'identifiant appartient à quelqu'un d'autre ? Personne ne l'avait remarqué parce que rien ne surveillait le décompte.
  - Le balayage montre que les douze répondent **`404`** et non `403` : un refus qui dit « vous n'avez pas le droit d'y toucher » confirme que la ligne existe, et un identifiant se devine facilement. Le douzième (`GET /api/products/:id/stock-counts`) répond `200 []`, ce qui est correct pour une liste — vérifié sérieusement plutôt que sur lecture du code : le produit de la victime porte un **vrai comptage**, que sa propriétaire voit et que l'attaquant ne voit pas, sans quoi le tableau vide ne prouverait rien. Le locataire ne peut jamais être choisi par l'appelant (un `locationId` dans le corps est rejeté par `strictObject`, dans la requête il est ignoré — décompte identique avec et sans), et la pagination ne permet pas d'aspirer la table (`limit=999999`, `limit=-1`, date malformée : `400`).
  - Deux effets de bord traités en cours de route. L'assertion de cardinalité d'`OPS-02` était **sur-spécifiée** : elle comptait les clés de route commençant par `/api/products/` au lieu de vérifier ce qui l'intéresse — qu'aucune clé ne porte d'identifiant brut — et le balayage inter-locataires, en appelant `/stock` et `/stock-counts`, l'a fait échouer sur un registre qui fonctionnait parfaitement. Et la suite e2e a franchi un seuil de charge à 128 tests : elle tourne contre **un seul** `next dev`, qui compile à la demande, et au-delà de deux workers il lâche des requêtes — le symptôme étant une connexion qui retombe silencieusement sur `/login` dans la spec la plus gourmande du moment. Mesuré plutôt que deviné : 7 workers → 2 à 4 tests perdus par exécution, 4 → 1 à 2, 2 → trois exécutions sur trois au vert. Le coût est ~25 s ; si la suite doit un jour aller plus vite, la réponse est une version de production derrière elle, pas plus de workers contre un serveur de développement.
  - Testé par `tests/e2e/isolation-p1.spec.ts` (5 cas) et, surtout, par un contrôle statique dans `tests/unit/architecture.test.ts` qui **énumère les routes prenant un identifiant et exige que chacune soit nommée dans le balayage** : c'est le garde de l'angle mort lui-même et pas seulement de ses conséquences, puisqu'un treizième endpoint serait passé exactement comme les onze précédents. Vérifié en ajoutant un endpoint factice — le test tombe avec la marche à suivre.

- [x] **`OPS-09` — Préparer les données et la procédure de pilote**
  - Priorité : `P1`
  ...
  - Mise en œuvre : `scripts/import-pilote.mjs` (+ `scripts/lib/csv-read.mjs`), `scripts/check-pilote.mjs` (+ `scripts/lib/pilote-core.mjs`), commandes `pnpm import:pilote` / `pnpm check:pilote`, et la procédure complète dans [`docs/pilote.md`](./docs/pilote.md).
  - **Import** : saisir quatre-vingts produits un par un n’est pas une mise en service. Tout passe par l’**API, connecté comme le propriétaire**, jamais par SQL — les règles qui rendent un catalogue correct vivent dans les services derrière ces endpoints (repli de TVA `DEC-05`, mouvement d’entrée qui maintient l’invariant `DEC-06`, audit, permissions), et un import qui écrirait en base devrait les réimplémenter puis s’en écarterait au premier changement. `--simulation` valide et affiche ligne par ligne sans rien écrire ; l’import est **idempotent par nom**, donc relancé après correction de trois lignes il ne crée pas un second « Terrasse 1 » ; les lignes invalides sont signalées avec leur numéro dans le fichier et n’interrompent pas les autres. Le mot de passe se lit dans l’environnement et jamais sur la ligne de commande, les arguments étant visibles de tout processus de la machine.
  - Un vrai bug trouvé par mon propre test du lecteur CSV : accepter `;` **et** `,` comme séparateurs simultanément coupait `2,50` en deux cellules — le produit aurait été tarifé **2 €** sans un mot de plainte. Le séparateur est désormais **détecté sur la ligne d’en-tête**, la seule ligne d’un fichier qui ne peut pas contenir de décimale. Le lecteur avale ce qu’un tableur français exporte réellement : marqueur d’octets d’Excel, `CRLF`, guillemets échappés, virgules et retours à la ligne dans une cellule, en-têtes avec majuscules/accents/espaces.
  - **Séparation démo/pilote**, la première moitié de l’acceptation : `FND-14` et la porte de configuration d’`OPS-05` sont **préventifs** — ils empêchent de *créer* la démo. Aucun ne répond à la question qu’un opérateur pose avant de livrer : est-ce que c’est là *maintenant* ? Une base restaurée depuis le dump d’un développeur passe les deux garde-fous et reste pleine de « Kalloud Lounge ». `check-pilote.mjs` regarde donc la **donnée** : il refuse les comptes et l’établissement de démonstration ainsi que les restes de tests (`Test table …`, `%@example.test`), et affiche ce que la base contient réellement pour que l’opérateur la reconnaisse. À relancer après chaque restauration.
  - **Responsable et canal**, la seconde moitié : astreinte tournante entre Younes Fafi, Samir Elbouzidi et Mehdi Ezka, canal d’incident sur un groupe WhatsApp dédié avec le gérant. Deux réserves écrites dans la procédure plutôt que passées sous silence : une rotation sans calendrier nominatif veut dire personne — chacun suppose qu’un autre a vu l’alerte —, et WhatsApp ne laisse **aucune trace exploitable**, donc chaque incident est transcrit le jour même dans un fichier partagé, sans quoi la revue de fin de pilote se fera de mémoire.
  - Testé par `tests/unit/csv-read.test.ts` (11 cas, dont la décimale française, vérifiée par mutation en réintroduisant le bug — trois tests tombent) et `tests/integration/pilote.test.ts` (6 cas : base migrée vide déclarée prête, base ne contenant que le vrai client déclarée prête, refus sur les comptes de démonstration, sur l’établissement de démonstration et sur les restes de tests, et le décompte du contenu). L’import a été exercé de bout en bout sur un CSV réaliste : invariant `DEC-06` vérifié sur les produits importés, et un second passage ne crée aucun doublon.

- [x] **`CLEAN-01` — Supprimer les chemins morts et doublonnés**
  - Priorité : `P1`
  ...
  - **Les quatre critères d'acceptation, vérifiés structurellement plutôt qu'à l'œil.** Un seul `INSERT INTO orders` dans tout le dépôt (`lib/repositories/tickets.ts`) : chaque vente passe donc par un ticket, ce qui *est* le cycle unique que `ORD-07` a instauré, et il n'existe plus aucun raccourci pour créer une commande payée. Un seul `INSERT INTO stock_movements` (`lib/repositories/stock-movements.ts`), le seed mis à part qui est un outil de développement. `database/demo-reset.sql` n'existe plus nulle part. **Aucun bouton sans effet** : tous les `<button>` des pages et composants ont un gestionnaire, une soumission explicite ou implicite — le seul cas signalé par mon balayage était un faux positif de mon expression régulière, qui s'arrêtait sur le `>=` d'un attribut.
  - Code mort supprimé après recensement de **toutes** les valeurs exportées du dépôt, tests inclus : `lib/client/use-currency.ts` en entier (`useCurrencyFormatter`, écrit pour `CFG-01` et jamais branché — chaque écran formate encore ses montants lui-même), `isRole` (`lib/authz.ts`), `listConfigurableTables` (`lib/services/configuration.ts`, une enveloppe d'une ligne que la route contourne en appelant directement le repository), `ticketTotalCents` et `formatTicketTotal` (`lib/services/tickets.ts`, écrits « pour les appelants qui comparent à une ventilation de paiement » — cette comparaison a fini ailleurs). Plus deux imports morts (`UpdateMemberBody`, `listCategories`).
  - Distingué au passage **code mort** et **sur-export** : 18 valeurs ne sont utilisées que dans leur propre fichier, mais elles y sont bien utilisées. Retirer leur `export` serait du remue-ménage, pas du nettoyage — `PAYMENT_METHODS` ou `discountSchema` sont le vocabulaire d'un module, et le livrable parle de chemins morts, pas d'hygiène d'export. Elles sont donc laissées telles quelles, sciemment.
